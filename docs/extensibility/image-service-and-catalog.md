...
description: 本文包含有关采用映像服务和映像目录Visual Studio指南和最佳做法。
...
ms.locfileid: "126600753"
---
# <a name="image-service-and-catalog"></a>映像服务和目录
本指南包含有关采用 2015 Visual Studio 中引入的映像服务和映像目录的指南Visual Studio最佳做法。
...
- 解决实现中的旧体系结构障碍
  ...
  使用Visual Studio之前和之后创建 shell 工具栏：
...
- Win32：HBITMAP
  ...
  **图像名字对象**
  ...
  图像名字对象 (或名字对象) 是 GUID/ID 对，用于唯一标识映像库中的图像资产或图像列表资产。

  **已知名字对象**

  映像目录中包含的映像名字对象Visual Studio，并公开由任何Visual Studio组件或扩展使用。

  **映像清单文件**

  *.imagemanifest (.imagemanifest*) 文件是 XML 文件，用于定义一组图像资产、表示这些资产的名字对象，以及代表每个资产的真实图像或图像。 映像清单可以定义独立映像或图像列表，以支持旧版 UI。 此外，还可以在资产上或每个资产后面的单个图像上设置属性，以更改显示这些资产时和方式。
 ...
 **ManifestToCode**
 ...
 Manifest to Code 工具获取一个映像清单文件，并生成一个包装文件，用于在代码 (c + +、c # 或 VB) 或 *.vsct* 文件中引用清单值。

 **ImageLibraryViewer**

 图像库查看器工具可以加载图像清单，并使用户能够以相同的方式处理它们 Visual Studio 确保正确编写清单。 用户可以更改背景、大小、DPI 设置、高对比度和其他设置。 它还显示加载信息以查找清单中的错误，并显示清单中每个图像的源信息。
...
- 我正在更新 CPS Project 系统。 **ImageName** 和 **StockIconService** 发生了什么情况？
  ...
  - 当 CPS 被更新为使用名字对象时，已将其删除。 不再需要调用 **StockIconService**，只需使用 CPS 实用工具中的 **ToProjectSystemType ()** 扩展方法将所需的 **KnownMoniker** 传递给方法或属性。 可以在下面找到 **ImageName** 到 **KnownMonikers** 的映射：
    ...
    |ImageName. WebConfig|KnownImageIds. Configurationfile.ini|
    |ImageName. Html 页|KnownImageIds. HTMLFile|
    |ImageName|KnownImageIds|
    |ImageName. ScriptFile|KnownImageIds.JSScript|
    |ImageName. TextFile|KnownImageIds|

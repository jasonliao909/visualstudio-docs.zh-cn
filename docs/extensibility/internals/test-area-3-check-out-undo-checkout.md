...
|操作|测试步骤|要验证的预期结果|
|------------|----------------|--------------------------------|
|) 客户端项目中查看专用 (COE|1. 创建客户端项目。<br />2. 将解决方案添加到源代码管理。<br />3. 以独占方式检查整个项目 (**文件**，请 **查看**) 。|检查发生。|
|) 文件系统或本地 IIS Web 项目查看独占 (COE|1. 将 Web 服务器连接设置为 " **工具**"、" **选项**"、" **项目**" 和 " **Web 设置**"。<br />2. 创建 Web 项目。<br />3. 将解决方案添加到源代码管理。<br />4.**仅查看 (** **文件**、**源代码管理**) 的整个项目。|检查发生。|
|查看解决方案中的解决方案项 (处理其他文件的新方法) |1. 创建空白解决方案。<br />2. 将解决方案添加到源代码管理。<br />3. 查看解决方案。<br />4. 添加若干解决方案项。<br />5. 检查所有新添加的项。<br />6. 选择多个解决方案项。<br />7. 查看选定项 (快捷菜单中， **查看**) 。|选择的文件已签出。|
|如果测试下的插件支持此功能，请签出本地版本 () |1. 用户1：创建客户端项目。<br />2. 用户1：将解决方案添加到源代码管理。<br />3. 用户2：从源代码管理打开解决方案到其他位置。<br />4. 用户2：签出文件。<br />5. 用户2：修改文件。<br />6. 用户2：签入文件。<br />7. 用户1：查看文件的本地版本 (选中 "**签出**" 对话框中的 "**签出本地版本** 高级" 选项) 。|文件的本地版本已签出。<br /><br /> 用户2所做的修改不适用于用户1文件。|
...
仅当连接到源代码管理存储时，才可以进行排他签出操作。 无论是连接还是断开连接，都可以随时执行共享的签出操作。 因此，当从版本存储区断开连接时，只会启用 **签出 Shared** (COS) 命令。 断开连接时，由于无法检索旧版本来替换用户所做的更改， **撤消签出** 已禁用。

当用户重新连接到版本存储区时，将同步所有已登记解决方案和项目的签出状态。 对于用户已执行的签出，此功能将执行所需的更新。 发生同步后，用户可以像正常 (连接) 一样继续工作。
...
|操作|测试步骤|要验证的预期结果|
|------------|----------------|--------------------------------|
|断开连接时，请查看文件，然后连接进行同步|1. 使用 "更改源代码管理" 对话框断开受控项目的连接 (**文件**、 **源代码管理**、 **更改源代码管理**) 。<br />2. 签出文件。<br />3. 在警告对话框中，单击 "签出 (断开连接) "。<br />4. 编辑该文件。<br />5. 使用 "更改源代码管理" 对话框进行连接。<br />6. 获取编辑过的文件的最新版本。|常见的预期行为|
...
|操作|测试步骤|要验证的预期结果|
|------------|----------------|--------------------------------|
|文件的无提示签出|1. 将 " **工具**"、" **选项**"、" **源代码管理** " 设置为 **在编辑时自动签出文件**。<br />2. 使用文件创建新的项目。<br />3. 将解决方案添加到源代码管理。<br />4. 查看文件。|文件将以无提示方式签出 (不) 的 UI。|
|项目的无提示签出|1. 将 " **工具**"、" **选项**"、" **源代码管理** " 设置为 **在编辑时自动签出文件**。<br />2. 创建一个新项目。<br />3. 将解决方案添加到源代码管理。<br />4. 查看项目。|文件将以无提示方式签出 (不) 的 UI。|
...
|将文件 () 添加到项目后，撤消项目的签出|1. 创建一个新项目并将其添加到源代码管理。<br />2. 签出项目。<br />3. 将文件添加到项目。<br />4. 撤消项目的签出。|添加的文件将从解决方案资源管理器中的项目中删除。<br /><br /> 项目不再处于签出状态。|
|从项目中删除文件 () 后撤消签出项目|1. 创建一个新项目并将其添加到源代码管理。<br />2. 签出项目。<br />3. 从项目中删除文件。<br />4. 撤消项目的签出。|删除的文件显示在解决方案资源管理器中的项目下。<br /><br /> 项目不再处于签出状态。|

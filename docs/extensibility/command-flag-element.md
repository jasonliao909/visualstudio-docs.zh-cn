...
|DynamicVisibility|可以通过 方法或 部分中包含的上下文 GUID 更改命令 `QueryStatus` 的 `VisibilityConstraints` 可见性。<br /><br /> 适用于显示在菜单和工具窗口工具栏上的命令，但不显示在主窗口上的顶级工具栏上的命令。 当从 方法返回顶级工具栏OLECMDF_INVISIBLE项时，可以禁用但不能隐藏 `QueryStatus` 这些项。 可以隐藏工具窗口工具栏上显示的工具栏命令。<br /><br /> 在菜单上，此标志还指示当其所有成员都隐藏时，它应自动隐藏。 此标志通常分配给子菜单，因为顶级菜单已具有此行为。<br /><br /> 此标志应与 标志 `DefaultInvisible` 结合使用。<br /><br /> 有效范围 `Button` `Combo` ：、、 `Menu`|

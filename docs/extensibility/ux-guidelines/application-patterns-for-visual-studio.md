...
在编辑器中使用的两种主窗口Visual Studio是文档编辑器和工具窗口。 大型无模式对话很少见，但可能。 尽管这些在 shell 中都是无模式的，但模式在根本上不同。 本部分介绍文档窗口、工具窗口和无模式对话的区别。 模式对话模式在对话 [中介绍](../../extensibility/ux-guidelines/application-patterns-for-visual-studio.md#BKMK_Dialogs)。

### <a name="comparing-window-usage-patterns"></a>比较窗口使用模式
**文档窗口** 几乎总是显示在文档井中。 这为文档编辑器提供了一个"中心阶段"来安排补充工具窗口。

**工具窗口** 通常显示为与 IDE 边缘折叠的单独较小窗口。 这可以是可见、隐藏或自动隐藏的。 但是，有时工具窗口通过取消选中窗口 **上的 Window/Docking** 属性在文档井中显示。 这可产生更多空间，但也是一个常见的设计决策：尝试集成到 Visual Studio 时，必须确定功能应显示工具窗口还是文档窗口。

**建议不要使用无** 模式Visual Studio。 根据定义，大多数无模式对话都是浮动工具窗口，应按此方式实现。 如果停靠在 shell 一侧的正常工具窗口的大小过于有限，则允许无模式对话。 如果用户可能将对话移动到辅助监视器，则也允许使用它们。
...
| **提交模型** | *延迟提交*<br /><br /> 为了将数据保存在文档中，用户必须发出"文件保存"、"另 **&gt;****存** 为"或"**全部保存"** 命令。 文档窗口包含数据的概念，即"已创建"，然后提交到其中一个保存命令。 关闭文档窗口时，所有内容都保存到磁盘或丢失。 | *立即提交*<br /><br /> 没有保存模型。 对于有助于编辑文件的检查器工具窗口，该文件必须在活动编辑器或设计器中打开，并且编辑器或设计器拥有保存。 | *延迟提交或立即提交*<br /><br /> 大多数情况下，大型无模式对话需要一个操作来提交更改，并允许执行"取消"操作，该操作会回滚在对话会话中进行的任何更改。  这区分了无模式对话框与工具窗口中的工具窗口始终具有即时提交模型。 |
...
| **实例** | *多实例*<br /><br /> 可以同时打开多个编辑器并编辑不同的文件，而某些编辑器还允许使用"窗口""新建窗口"命令 (多个编辑器中打开同一) 。 **&gt;**<br /><br /> 单个编辑器可以同时编辑一个或多个文件， (Project设计器) 。 | *单实例或多实例*<br /><br /> 内容会更改以反映上下文 (如属性浏览器) 或将焦点/上下文推送到其他窗口 (任务列表解决方案资源管理器) 。<br /><br /> 除非有令人信服的理由，否则单实例和多实例工具窗口都应与活动文档窗口相关联。 | *单实例* |
...
### <a name="overview"></a>概述
工具窗口支持在文档窗口中发生的用户工作。 它们可用于显示一个层次结构，该层次结构表示由用户提供Visual Studio操作的基本根对象。
...
- **停靠/固定的工具** 窗口可以附加到文档区域四侧的任何一侧。 图钉图标将显示在工具窗口标题栏中。 工具窗口可以沿 shell 和其他工具窗口的边缘水平或垂直停靠，也可以进行选项卡链接。

- **自动隐藏的工具** 窗口未固定。 窗口可以滑出视线，使选项卡 (工具窗口的名称及其图标) 位于文档区域的边缘。 用户将鼠标悬停在选项卡上时，工具窗口会滑出。
...
工具窗口是单实例或多实例。 某些单实例工具窗口可能与活动文档窗口相关联，而多实例工具窗口可能没有关联。 多实例工具窗口通过创建 **窗口 &gt; 的新** 实例来响应"窗口新建窗口"命令。 下图演示了在窗口实例处于活动状态时启用"新建窗口"命令的工具窗口：

![当窗口实例处于活动状态时启用"新建窗口"命令的工具窗口](../../extensibility/ux-guidelines/media/0702-02_toolwindowenablingcommand.png "0702-02_ToolWindowEnablingCommand")<br />当窗口实例处于活动状态时启用"新建窗口"命令的工具窗口

可以隐藏或显示单实例工具窗口，而多实例工具窗口可以关闭或隐藏。 所有工具窗口都可以停靠、选项卡链接、浮动或设置为 Multiple-Document 接口 (MDI) 子窗口 (类似于文档窗口) 。 所有工具窗口都应响应"窗口"菜单中相应的窗口管理命令：

!["窗口"菜单中Visual Studio管理命令](../../extensibility/ux-guidelines/media/0702-03_windowmanagementcontrols.png "0702-03_WindowManagementControls")<br />"窗口"菜单中Visual Studio管理命令

#### <a name="document-specific-tool-windows"></a>特定于文档的工具窗口
某些工具窗口旨在根据给定类型的文档进行更改。 这些窗口会不断更新，以反映适用于 IDE 中活动文档窗口的功能。

其内容更改以反映所选编辑器的工具窗口示例包括"工具箱"和"文档大纲"。 当编辑器具有无法向窗口提供上下文的焦点时，这些窗口会显示水印。

#### <a name="navigable-list-tool-windows"></a>可导航列表工具窗口
某些工具窗口显示用户可与之交互的可导航项的列表。 在此类型的窗口中，应始终提供列表中当前项的反馈，即使窗口处于非活动状态。 列表还应通过更改窗口中当前选定的项来响应 **GoToNextLocation** 和 **GoToPrevLocation** 命令

可导航列表工具窗口的示例包括解决方案资源管理器和"查找结果"窗口。
...
| 工具窗口 | 函数 |
| --- | --- |
| “解决方案资源管理器” | 一个层次结构树，显示项目、杂项文件和解决方案项中包含的文档列表。 项目中项的显示由拥有项目类型的包定义 (例如，基于引用、基于目录或混合模式) 。 |
| 类视图 | 独立于文件本身的工作文档集内类和各种元素的分层树。 |
| 服务器资源管理器 | 显示解决方案中所有服务器和数据连接的分层树。 |
| 文档大纲 | 活动文档的层次结构。 |
...
| 工具窗口 | 函数 |
| --- | --- |
| 属性 | 一个网格，显示所选对象的属性列表，以及用于编辑这些属性的值选取器。 |
...
| 工具窗口 | 函数 |
| --- | --- |
| 帮助 | 一个窗口，允许用户从"如何？"访问各种获取帮助的方法 视频到 MSDN 论坛。 |
| 动态帮助 | 一个工具窗口，显示指向适用于当前所选内容的帮助主题的链接。 |
| 对象浏览器 | 一个双列框架集，其左侧窗格中有分层对象组件的列表，右侧列包含对象的属性和方法。 |

**对话框工具窗口**

| 工具窗口 | 函数 |
| --- | --- |
| 查找 | 一个对话框，允许用户在解决方案内的各种文件中查找或替换 。 |
| 高级查找 | 一个对话框，允许用户在解决方案内的各种文件中查找或替换 。 |
...
| 工具窗口 | 函数 |
| --- | --- |
| 工具箱 | 用于存储将拖放到设计图面上的元素的工具窗口，为所有设计器提供一致的拖动源。 |
| 起始页 | 用户要访问的Visual Studio，可以访问开发人员新闻源、Visual Studio帮助和最近的项目。 用户还可以创建自定义起始页，将 StartPage.xaml 文件从"Common7\IDE\StartPages Visual Studio 程序文件"目录复制到 Visual Studio 文档目录中的 StartPages 文件夹，然后手动编辑 XAML 或在 Visual Studio 或其他代码编辑器中打开它。 \" |
...
"文档井"是 IDE 中最大的空间，用户通常会在补充工具窗口的辅助下专注于完成任务。 文档编辑器表示用户打开的基本工作单元，并保存在Visual Studio。 它们保留了与活动层次结构窗口或其他活动解决方案资源管理器紧密相关的选择感。 用户应该能够指向其中一个层次结构窗口，并知道文档包含的位置及其与解决方案、项目或由 Visual Studio 包提供的另一个根对象的关系。

文档编辑需要一致的用户体验。 若要允许用户专注于当前任务，而不是窗口管理和查找命令，请选择最适合用户任务的文档视图策略，以便编辑该文档类型。

#### <a name="common-interactions-for-the-document-well"></a>文档井的常见交互

- 在常见的"新建文件和打开文件"体验 **中保持** 一致的 **交互** 模型。

- 在文档窗口打开时更新相关窗口和菜单中的相关功能。

- 菜单命令已适当集成到常见菜单，如"编辑"菜单 **、"格式**"和"**视图"** 菜单。 如果有大量专用命令可用，则可以创建一个新菜单。 只有当文档具有焦点时，才应显示此新菜单。
...
#### <a name="size"></a>大小
对话框应不超过 1024x768 屏幕分辨率，初始对话大小不应超过 900x700 像素。 对话框可以调整大小，但这不是一项要求。
...
- 标题栏中的帮助按钮已弃用。 请勿将它们添加到新对话框中。 它们存在时，应启动与任务相关的概念帮助主题。
  ...
  ![对话框标题栏Visual Studio规范](../../extensibility/ux-guidelines/media/0704-03_titlebarspecs.png "0704-03_TitleBarSpecs")<br />对话框中标题栏Visual Studio规范
...
| --- | --- | --- |
| Tab 控件 | 以逻辑方式将对话框页分组到相关集<br /><br />对于少于五个 (或对话框中一行中容纳的选项卡数非常有用，) 对话框中相关控件的页面<br /><br />选项卡标签必须短：一个或两个可以轻松识别内容的单词<br /><br />通用系统对话样式<br /><br />示例 **：文件资源管理器 &gt; 项属性** | 制作描述性短标签可能很困难<br /><br />通常不会在一个对话框中缩放过去五个选项卡<br /><br />如果一行的选项卡过多，则 (另一种分层技术) <br /><br />不可扩展 |
| 边栏导航 | 可以容纳比选项卡更多的类别的简单切换设备<br /><br />没有层次结构的 (列表) <br /><br />可扩展<br /><br />示例： **自定义... &gt; 添加命令** | 如果少于三个组，则不能充分利用水平空间<br /><br />任务可能更适合下拉列表 |
| 树控件 | 允许无限制的类别<br /><br />允许对类别进行分组和/或层次结构<br /><br />可扩展<br /><br />示例： **工具 &gt; 选项** | 高度嵌套的层次结构可能导致过多的水平滚动<br /><br />Visual Studio树视图的过度使用 |
...
仪表板和仪表板是充当其他对话框和窗口的启动点的对话框或面板。 设计良好的"浮点"会立即显示最常见的选项、命令和设置，允许用户轻松完成常见任务。 就像现实世界中的空调提供用于访问它们后面房间的门一样，此处不太常见的 UI 被收集到单独的"房间"中 (通常还有一些对话) 这些对话包含一些相关功能，这些功能可以从主要空调访问。

或者，在单个集合中提供所有可用功能的 UI，而不是将不太常见的功能重构到单独的位置，只是一个仪表板。

![在应用公开其他 UI 的Outlook](../../extensibility/ux-guidelines/media/0704-08_hallway.png "0704-08_Hallway")<br />用于公开其他 UI 的Outlook
...
项目通常将自身分类为基于引用的 (只能保留对存储) 中的项目项的引用、基于目录的 (只能持久保存实际存储在项目层次结构) 中的项目项，或能够保留引用或物理项) 的混合 (。 IDE 在 解决方案资源管理器 中同时容纳所有 **三解决方案资源管理器。**
...
- 当 Ctrl 和 Shift解决方案资源管理器 (都不应导致移动操作时，) 中未修改的拖动操作。

- 移位拖动操作还应导致移动操作。

- Ctrl 拖动操作应会导致复制操作。

- 基于引用的项目系统和混合项目系统支持将链接 (或引用) 源项的概念。 当这些项目是拖放操作的目标时 (当 **Ctrl + Shift**) 时，它应导致对添加到项目的项的引用
...
若要正确实现拖放行为，拖动的源项目需要将其性质传达给目标项目。  (例如，它是基于引用还是基于目录) 此信息由源提供的剪贴板格式指示。 作为拖动 (或剪贴板复制操作) 项目应分别提供 或 ，具体取决于项目是基于引用还是 `CF_VSREFPROJECTITEMS` `CF_VSSTGPROJECTITEMS` 基于目录。 这两种格式具有相同的数据内容，这类似于 Windows 格式，只不过字符串列表（而不是文件名 `CF_HDROP` `NULL` `Projref` ） `IVsSolution::GetProjrefOfItem` `::GetProjrefOfProject` 是字符串的双终止列表 (如从 或 返回的或) 。
...
| 修饰符 | Category | 源项：引用/链接 | 源项：物理项或文件系统 (`CF_HDROP`)  |
| --- | --- | --- | --- |
| No 修饰符 | 操作 | 移动 | 链接 |

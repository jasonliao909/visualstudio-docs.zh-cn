...
|字符串|必需。 请参阅 [字符串元素](../extensibility/strings-element.md)。 必须 `ButtonText` 定义子元素。|
|Annotation|可选注释。|

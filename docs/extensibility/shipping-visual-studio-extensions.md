---
title: 装运 Visual Studio 扩展 |Microsoft Docs
...
## <a name="working-with-vsix-extensions"></a>使用 VSIX 扩展
 您可以通过创建一个空白 vsix Project，然后向其添加不同的项模板来创建 vsix 扩展。 有关详细信息，请参阅[VSIX Project 模板](../extensibility/vsix-project-template.md)。

 你可以使用 VSIX 格式将项目模板、项模板、vspackage、Managed Extensibility Framework (MEF) 组件、**工具箱** 控件、程序集和自定义类型打包 (其中包括 Visual Studio 2017) 的自定义起始页。 VSIX 格式使用基于文件的部署。 有关 VSIX 包的详细信息，请参阅 [Vsix 包的解析](../extensibility/anatomy-of-a-vsix-package.md)。
...
## <a name="publishing-your-extension-to-the-visual-studio-marketplace"></a>将扩展发布到 Visual Studio Marketplace
 只需通过向用户发送 .vsix 文件或将其放在服务器上，即可将扩展分发给其他人。 但很多人都要将您的代码放在[Visual Studio Marketplace](https://marketplace.visualstudio.com/vs)中。 Visual StudioMarketplace 扩展可通过 **扩展和更新** Visual Studio 用户使用。 有关详细信息，请参阅[查找和使用 Visual Studio 扩展](../ide/finding-and-using-visual-studio-extensions.md)。
...
## <a name="updating-and-versioning-your-extension"></a>更新扩展并对其进行版本控制
 ...
 你可以设置扩展以支持 Visual Studio 的多个版本。 有关详细信息，请参阅[支持多个版本的 Visual Studio](../extensibility/supporting-multiple-versions-of-visual-studio.md)。
...
|[如何：更新扩展](../extensibility/how-to-update-a-visual-studio-extension.md)|介绍如何更新系统上的扩展，以及如何将更新部署到现有 Visual Studio 扩展。|
...
|[支持多个版本的 Visual Studio](../extensibility/supporting-multiple-versions-of-visual-studio.md)|演示如何使你的扩展支持 Visual Studio 的多个版本。|
|[查找 Visual Studio](locating-visual-studio.md)|描述如何查找自定义扩展部署 Visual Studio 实例。|

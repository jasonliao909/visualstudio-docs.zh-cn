...
ms.locfileid: "112308559"
---
# <a name="update-a-visual-studio-extension-for-visual-studio-2022"></a>更新 visual studio 2022 的 Visual Studio 扩展

可以按照本指南中所示更新扩展以使用 Visual Studio 2022 Preview。 Visual Studio 2022 Preview 是64位应用程序，并在 VS SDK 中引入了一些重大更改。 本指南将指导你完成以下步骤：使用 Visual Studio 2022 的当前预览版来处理你的扩展，以便在 Visual Studio 2022 达到 GA 之前，你的扩展可供用户安装。

## <a name="installing"></a>安装

从 [Visual studio 2022 预览版下载](https://visualstudio.microsoft.com/vs/preview/vs2022)安装 visual Studio 2022 预览版。

### <a name="extensions-written-in-a-net-language"></a>用 .NET 语言编写的扩展

面向托管扩展的 Visual Studio 2022 的 VS SDK *专门* 在 NuGet 上运行：

- VisualStudio) 元包中的[](https://www.nuget.org/packages/Microsoft.VisualStudio.Sdk/) (版本 17. x 版本引入了所需的大多数或所有引用程序集。
- 应从 VSIX 项目引用 [VSSDK.](https://www.nuget.org/packages/Microsoft.VSSDK.BuildTools/) x (版本) 包的 BuildTools，以便它能够生成符合 Visual Studio 2022 的 vsix。

*必须* 用 "Any CPU" 或 "x64" 平台编译扩展。 "X86" 平台与 Visual Studio 2022 的64位进程不兼容。

### <a name="extensions-written-in-c"></a>用 c + + 编写的扩展

与往常一样，已安装的 Visual Studio SDK 提供了 VS SDK for extension with c + + 编译的。

扩展 *必须* 专门针对 Visual STUDIO 2022 SDK 和 amd64 进行编译。
...
了解如何将 Visual Studio 2022 预迁移扩展迁移到 Visual Studio 2022：

1. [使项目实现现代化](#modernize-your-vsix-project)。
1. 将[您的源代码重构为共享项目](#use-shared-projects-for-multi-targeting)，以允许以 Visual Studio 2022 和更早的版本为目标。
...
> [!NOTE]
> 如果要使用 Visual Studio 2022 Preview 创作 *新* 的 visual studio 扩展，并想要 () 面向 Visual studio 2019 或更早版本，请参阅 [本指南](target-previous-versions.md)。

### <a name="msbuild-tasks"></a>MSBuild 任务

如果你创作 MSBuild 任务，请注意，在 Visual Studio 2022 中，可能会将其加载到64位 MSBuild.exe 进程。 如果任务需要32位进程才能运行，请参阅 [此 MSBuild 文档](../../msbuild/how-to-configure-targets-and-tasks.md#usingtask-attributes-and-task-parameters) ，以确保 msbuild 知道在32位进程中加载任务。

## <a name="modernize-your-vsix-project"></a>实现 VSIX 项目的现代化

向扩展添加 Visual Studio 2022 支持之前，强烈建议您在继续操作之前，先清理并现代化现有项目，其中包括：
...
1. 将任何直接程序集 VS SDK 程序集引用替换为 PackageReference 项。
   ...
   某些对 VS SDK 不唯一的程序集 (例如，2022在 visual Studio 2022 之前，Newtonsoft.Json.dll) 可能已通过简单引用发现， `<Reference Include="Newtonsoft.Json" />` 但在 visual studio 中，需要包引用，因为在 Visual studio 2022 中，我们从 MSBuild 的默认程序集搜索路径中删除了一些 Visual studio 运行时和 SDK 目录。
...
[共享项目](/xamarin/cross-platform/app-fundamentals/shared-projects?tabs=windows) 是在 Visual Studio 2015 中引入的一种项目类型。 Visual Studio 中的共享项目允许在多个项目之间共享源代码文件，并使用条件编译符号和唯一的引用集以不同的方式进行生成。

由于 Visual Studio 2022 需要所有以前的 VS 版本中的一组不同的引用程序集，因此我们的指导是使用共享项目方便多个目标扩展到 Visual Studio 2022 和 Visual Studio 2022 (及更高) 版本的扩展，并提供代码共享但不同的引用。

在 Visual Studio 扩展的上下文中，可以为 Visual Studio 2022 和更高版本使用一个 VSIX 项目，为 Visual Studio 2019 和更早版本创建一个 VSIX 项目。 其中每个项目都只包含一个 `source.extension.vsixmanifest` ，包引用的是对 16. x sdk 或 17. x sdk 的引用。 这些 VSIX 项目还会对新的共享项目进行共享项目引用，该共享项目将托管可以在两个版本之间共享的所有源代码。

为此，我们假设你已有一个面向 Visual Studio 2019 的 VSIX 项目，并且你希望你的扩展在 Visual Studio 2022 上运行。
...
1. 为每个引用 VS SDK 的现有项目，将新的共享项目添加到解决方案。
   ![添加新项目命令 ](media/update-visual-studio-extension/add-new-project.png)
...
使用以下步骤继续向扩展添加 Visual Studio 2022 支持，这些步骤可使用 Visual Studio 2019 完成：

1. 向解决方案添加新的 VSIX 项目。 这将是面向 Visual Studio 2022 的项目。 删除模板附带的任何源代码，但 *保留该 `source.extension.vsixmanifest` 文件*。

1. 在新的 VSIX 项目上，将共享项目引用添加到 Visual Studio 2019 目标 VSIX 引用所在的同一个共享项目。
...
1. 对于托管的 VS 扩展，使用 NuGet 包管理器或直接编辑项目文件，将 (或更早) 版本的包引用更新为 Visual Studio 2022 目标项目文件中的 17. x 包版本：
...
1. 编辑 `source.extension.vsixmanifest` 文件以反映 Visual Studio 2022。 设置 `<InstallationTarget>` 标记以反映 Visual Studio 2022 并指示 amd64 负载：
   ...
   在 Visual Studio 2019 中，此文件的设计器不会公开新 `ProductArchitecture` 元素，因此，需要使用 xml 编辑器来完成此更改，可以通过 **解决方案资源管理器** 中的 "**打开方式**" 命令访问该编辑器。

   此 `ProductArchitecture` 元素是关键元素。 Visual Studio *2022 不会安装你* 的扩展。
...
此时，你有一个面向 2022 Visual Studio VSIX 的扩展。 应生成面向 2022 Visual Studio VSIX 项目，并处理出现的任何 [生成中断](#handle-breaking-api-changes)。 如果在面向 2022 Visual Studio VSIX 项目中没有生成中断，祝贺你：你已准备好进行测试！

## <a name="handle-breaking-api-changes"></a>处理中断性 API 更改

2022 [年](breaking-api-list.md) Visual Studio API 发生了中断性变更，可能需要在早期版本中运行代码时更改代码。 查看该文档，获取有关如何更新每个代码的代码的提示。
...
可以使用 Visual Studio 2022 预览版来生成和测试扩展，无论它们Visual Studio 2022 或更早版本。 从 2022 Visual Studio启动 VSIX 项目时，Visual Studio启动一个试验实例。
...
将扩展发布到 [Visual Studio Marketplace](https://marketplace.visualstudio.com/) 是让新用户查找和安装扩展的一种好方法。 无论扩展是Visual Studio 2022 年版本还是面向较旧的 VS 版本，市场都支持你。
...
对于 2022 Visual Studio预览版，市场将仅支持每个市场列表的单个 VSIX 文件。 尽管 Visual Studio 2022 年目前为预览版，但我们建议你为扩展单独Visual Studio 2022 年市场列表。 这样，你可根据需要Visual Studio 2022 扩展，而不会影响客户对早期版本的 Visual Studio。 还可以将扩展标记为"预览"，以设置这样一种预期：即使该不可靠性的来源是 2022 年 2 月Visual Studio，它的可靠性也可能低于主流扩展。
...
可以通过 LAN 或其他任何方式共享扩展。 如果面向 Visual Studio 2022 和 Visual Studio 2022 之前版本，则需要单独共享多个 VSIX，并赋予它们文件名 (或将其放在唯一文件夹中) 以帮助用户根据已安装的 Visual Studio 版本了解要安装的 VSIX。

---
title: 故障排除 SharePoint 解决方案|Microsoft Docs
...
---
# <a name="troubleshoot-sharepoint-solutions"></a>解决方案SharePoint疑难解答
...
## <a name="new-non-english-site-definitions-do-not-appear-in-site-creation-page-after-deployment"></a>部署后，新的非英语站点定义不会出现在站点创建页中
 使用非英语版本的 (（即区域设置不是 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] [!INCLUDE[TLA2#tla_id](../sharepoint/includes/tla2sharptla-id-md.md)] 1033) 的版本）创建和部署站点定义后，"SharePoint 自定义项"选项卡不会出现在"模板选择"框中，并且新站点模板不会出现在"新建 SharePoint 站点 **"页** 中。
...
## <a name="user-must-refresh-application-page-in-browser-while-debugging-to-view-updated-image"></a>用户调试时必须在浏览器中刷新应用程序页，以查看更新后的图像
 如果要调试包含SharePoint页的应用程序页的一个解决方案（如图像控件）的控件，则必须刷新浏览器中的页面以显示对图像进行的任何 [!INCLUDE[TLA2#tla_html](../sharepoint/includes/tla2sharptla-html-md.md)] 更改。

## <a name="error-the-site-location-is-not-valid"></a>错误：站点位置无效
 如果未安装 ， [!INCLUDE[moss_14_short](../sharepoint/includes/moss-14-short-md.md)] 则可能会出现此问题。 如果对自定义向导 中指定的SharePoint网站没有管理员访问权限 **，SharePoint也可能发生此错误**。
...
- 确保你具有对 SharePoint 网站的管理员访问权限。 有关详细信息，请参阅联机文章在 SharePoint 服务器 中分配 [!INCLUDE[TLA2#tla_office](../sharepoint/includes/tla2sharptla-office-md.md)] [或删除服务应用程序的管理员](/sharepoint/administration/assign-or-remove-administrators-of-service-applications)。
...
## <a name="deployment-error-appears-after-the-name-of-an-identifier-in-a-business-data-connectivity-model-project-is-changed"></a>更改业务数据连接模型项目中标识符的名称后，会出现部署错误
 如果在 BDC 模型的业务数据连接中更改实体的标识符 (，) 尝试部署解决方案，则会出现此问题。
...
1. 在 **解决方案资源管理器** 中，选择其文件扩展名为 *.ascx 的用户控件文件*。
...
## <a name="error-appears-when-an-imported-reusable-workflow-that-contains-a-task-form-field-is-run"></a>运行包含任务窗体字段的导入可重用工作流时出现错误
 如果导入包含具有 字段的任务窗体的工作流，然后在导入该工作流的同一系统运行新工作流，则会出现此问题。

### <a name="error-message"></a>错误消息
 部署步骤"激活功能"中出错：在当前网站集或子站点中找到了功能 [*Guid*] 中定义的 ID 为 [*Guid*] 的字段。

### <a name="resolution"></a>解决方法
 此错误是由于 中的"导入可重用工作流"项目不会更改任务窗体字段 ID 而发生的字段 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] ID 冲突的结果。 如果在包含原始工作流的同一服务器上部署导入的工作流，则会发生字段 ID 冲突。

 若要解决此问题，请使用"查找和替换"功能更改所有导入的工作流文件中"字段 ID"属性的值。

## <a name="error-appears-when-a-renamed-imported-list-instance-is-run"></a>运行重命名的导入列表实例时显示错误
 如果重命名导入的列表实例，然后在 中运行它，则会出现此问题 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 。

### <a name="error-message"></a>错误消息
 生成错误：部署步骤"激活功能"中出错：文件 Template\Features \\ [*import project*<em>feature</em>*name*]\Files\Lists \\ [*old*<em>list name</em>]\Schema.xml不存在。

### <a name="resolution"></a>解决方法
 导入列表实例时，名为 CustomSchema 的属性将添加到Elements.xml实例的表文件。 Elements.xml包括列表实例的自定义schema.xml路径。 在 中重命名列表实例时，自定义实例的部署 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] schema.xml更改，但 CustomSchema 属性的路径值不会更新。 因此，当激活该功能时，列表实例schema.xml CustomSchema 属性指定的旧路径中查找该文件。

 若要解决此问题，请更新 CustomSchema 属性中schema.xml *文件的部署* 位置的路径。

## <a name="sharepoint-debugging-session-terminated-by-iis"></a>SharePoint IIS 终止的调试会话
 如果在解决方案中设置断点，SharePoint F5 键运行该断点，然后保留在超过 90 秒的断点处，则会出现 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 此问题。 

### <a name="error-message"></a>错误消息
 正在调试的 Web 服务器进程已被 IIS Internet Information Services (终止) 。 您可通过在 IIS 中配置应用程序池 Ping 设置来避免此问题。 有关更多详细信息，请参阅帮助。

### <a name="resolution"></a>解决方法
 默认情况下，IIS 应用程序池等待 90 秒，应用程序在关闭应用程序之前做出响应。 此过程称为"ping"应用程序。 若要解决此问题，可以增加等待时间或完全禁用应用程序 ping。

##### <a name="to-access-the-iis-app-pool-settings"></a>访问 IIS 应用池设置

1. 打开 IIS 管理器。

2. 在"**连接"** 窗格中，展开SharePoint节点，然后选择"**应用程序池"** 节点。

3. 在"**应用程序** 池"页上，SharePoint应用程序池 (通常为"SharePoint - 80") ，然后在"操作"窗格中选择"高级 **设置"链接**。 

4. 若要增加 IIS 超时前的等待时间，将 Ping 最大响应时间 (**秒**) 更改为大于 90 秒的值。

5. 若要禁用 IIS ping，将 **"启用 Ping"设置为****"False"。**

## <a name="auto-retract-leaves-orphaned-list-instance-in-sharepoint"></a>自动收回将孤立列表实例保留SharePoint
 如果执行以下步骤，会出现此问题。

1. 创建在 中具有列表实例的列表定义 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 。

2. 选择 **F5** 键以运行解决方案。

3. 停止调试或关闭SharePoint站点。

4. 重新打开SharePoint站点并打开列表实例。

### <a name="error-message"></a>错误消息
 '/' 应用程序中出现服务器错误。

### <a name="resolution"></a>解决方法
 发生这种情况的原因是，关闭解决方案SharePoint会话后，自动收回功能会收回解决方案。 收回会从 SharePoint删除列表定义，但不删除列表的实例。 列表实例需要基础列表定义。

 若要解决此问题，请通过选择菜单栏上的"生成部署"**来部署**  >  **解决方案**。  (不要通过选择 **F5** 键来调试解决方案。) 然后，删除列表中的SharePoint。

## <a name="original-sharepoint-solution-is-replaced-by-an-exported-version"></a>原始SharePoint解决方案替换为导出的版本
 如果导出SharePoint解决方案，将解决方案导入 ，然后将解决方案部署回导出它的同一站点，将替换SharePoint [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] 解决方案。 如果将解决方案部署到未激活原始解决方案的服务器，则不会发生此问题。

### <a name="error-message"></a>错误消息
 无。

### <a name="resolution"></a>解决方法
 若要避免覆盖从其中导出解决方案的站点上的解决方案，请更改项目中所有导入功能的 SolutionID 的 GUID 和功能 [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] ID。

## <a name="error-appears-when-debugging-starts"></a>启动调试时出现错误
 在 Visual Studio 中开始调试 SharePoint 解决方案时出现错误，表明由于字典中没有给定键，Visual Studio 无法加载 Web.config 文件。

### <a name="error-message"></a>错误消息
 无法加载Web.config配置文件。 检查文件中是否有格式错误的 XML 元素，然后重试。 发生以下错误：字典中不存在给定键。

### <a name="resolution"></a>解决方法
 若要解决此问题，请确保 Visual Studio 中的 SharePoint 项目的“站点 URL”属性值与分配给 Web 应用程序的备用访问映射的默认区域的 URL 一致。 对 URL 使用其他区域（如 Intranet）将无法解决此错误。 项目的站点 URL 与默认区域中的 URL 必须一致。 若要访问备用访问映射，请打开 SharePoint 2010 管理中心实用工具，选择"应用程序管理"链接，然后在 **"Web** 应用程序"下选择"配置 **备用访问映射"** 链接。 有关详细信息，请参阅为 [Web 应用程序创建区域](/previous-versions/office/sharepoint-2007-products-and-technologies/cc263087(v=office.12))。

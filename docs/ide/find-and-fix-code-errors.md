...
ms.workload:
- multiple
ms.openlocfilehash: c1319d66db16863a10442b7d001b315bddcca573
ms.sourcegitcommit: 864b18011ff12d880584173c86dccfdcc425808f
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/01/2022
ms.locfileid: "141347762"
---
...
:::image type="content" source="media/debug-build-solution-menu-item.png" alt-text="IDE 中&quot;生成&quot;Visual Studio屏幕截图。":::

::: moniker-end

::: moniker range="vs-2017"

!["生成项目Visual Studio选项的屏幕截图。](../ide/media/vs_ide_gs_debug_build_menu_item.png)
...
![成功生成Visual Studio的屏幕截图。](../ide/media/vs_ide_gs_debug_success_build.png)
...
![输出和Visual Studio列表的屏幕截图。](../ide/media/vs_ide_gs_debug_bad_build_error_list.png)

单击“错误列表”窗口中的错误行，跳转至发生错误的行。 （或按 Ctrl+Q ，键入行号，然后从结果中选择“打开或关闭行号”来打开行号。 这是转至“选项”对话框最快的方法，可以在此打开行号。）

![带行号Visual Studio编辑器的屏幕截图。](../ide/media/vs_ide_gs_debug_line_numbers.png)

!["行号Visual Studio屏幕截图。](../ide/media/vs_ide_gs_debug_options_line_numbers.png)

按 Ctrl+G 快速跳转至发生错误的行号 。

错误由红色的“波形曲线”下划线标识。 将鼠标悬停在下划线上以获取其他详细信息。 修复后下划线消失，尽管纠正过程中可能会引入新的错误。 （此过程被称为“回归”。）

![错误悬停Visual Studio屏幕截图。](../ide/media/vs_ide_gs_debug_error_hover1.png)

遍历错误列表，并解决代码中的所有错误。

!["调试错误Visual Studio窗口的屏幕截图。](../ide/media/vs_ide_gs_debug_error_list.png)
...
![搜索时Visual Studio错误列表必应屏幕截图。](../ide/media/vs_ide_gs_debug_error_list_error_help.png)
...
!["分析Visual Studio Code项的屏幕截图。](../ide/media/vs_ide_gs_debug_run_code_analysis.png)

任何新的或更新的警告都将显示在 IDE 底部的“错误列表”选项卡中。 单击警告以跳转到代码中相应的行。

![显示警告Visual Studio错误列表的屏幕截图。](../ide/media/cpp-code-analysis-warning.png)
...
![灯泡预览Visual Studio屏幕截图。](../ide/media/quick-actions-options.png)

只要代码分析器确定有机会修复、重构或改进代码，就能使用“快速操作”。 单击任意代码行，右键单击以打开上下文菜单，然后选择“快速操作和重构”。 如果重构或改进选项可用，则会显示。 否则，IDE 的左下角会显示消息“此处无可用的快速操作”。

!["无可用快速操作"文本的屏幕截图。](../ide/media/vs_ide_gs_debug_light_bulb_no_options.png)
...
!["自动Visual Studio调用堆栈"窗口的屏幕截图。](../ide/media/vs_ide_gs_debug_autos_and_call_stack.png)

按 Shift+F5 或单击“停止”按钮停止应用  。 或者，可以仅关闭应用的主窗口（或命令行对话框）。

如果你的代码按预期完美而准确地运行，祝贺你！ 但是，如果它停止响应、崩溃了，或带来一些奇怪的结果，则需要找到这些问题的根源并修复 bug。
...
![Visual Studio 断点的屏幕截图。](../ide/media/vs_ide_gs_debug_breakpoint1.png)

断点的常见用途包括：

- 若要缩小故障或无响应程序的源范围，可将断点分散在你认为引起故障的方法调用的代码中。 在调试器中运行代码时，请移除断点，然后重设断点使其更紧密，直到找到有问题的代码行。 请参阅下一部分，了解如何在调试器中运行代码。
...
![Visual Studio 代码窗口的屏幕截图。 左侧滚动条中的红点指示以黄色标记的代码行中的断点。](../ide/media/vs_ide_gs_debug_hit_breakpoint.png)

在上图中，可以按 F10 或 F11 使调试器前移一条语句（由于此处没有方法调用，因此这两个命令的结果相同）。

调试器暂停时，可以检查变量并调用堆栈来确定所出现的情况。 这些值是否在你期望的范围内？ 调用是否正按正确的顺序进行？

![Visual Studio 代码窗口的屏幕截图。 在以黄色标记的代码行中，选中了一个变量，且下拉列表显示其当前值和引用。](../ide/media/vs_ide_gs_debug_inspect_value.png)

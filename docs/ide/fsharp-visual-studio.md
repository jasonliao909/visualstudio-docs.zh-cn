...
|ASP.NET 客户端服务|否||
|ClickOnce|否|以其他 .NET 语言使用客户端项目（如果适用）。|
|强命名|否|通过编译器命令行选项提供。|
|程序集发布和版本控制|否||
...
|构建|[“项目设计器”->“生成”页](reference/build-page-project-designer-csharp.md)|支持控制代码的编译方式。|
...
|“项目设计器”页|设置|描述|
| - |-------|-----------|
|构建|生成尾调用|选中后即可使用 Microsoft 中间语言 (MSIL) 尾指令。 这将导致对尾递归函数重复使用堆栈帧。 等效于 `--tailcalls` 编译器选项。|
|构建|其他标志|允许指定其他编译器命令行选项。|
...
|转到 .NET API 的定义|将光标置于 .NET API 时，显示 .NET 元数据生成的代码。|否|

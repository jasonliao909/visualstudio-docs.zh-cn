...
MSBuild 依赖于导入顺序，目标的最后一个定义是使用的定义。 如果尝试重新定义目标，则在以后定义内置目标时，它将不会生效。 对于使用 SDK 的项目，定义顺序并不一定明显，因为目标的导入是在项目文件的末尾隐式添加的。

因此，要扩展现有目标的行为，请创建新的目标并指定 `BeforeTargets` 或 `AfterTargets`（视情况而定），如下所示：

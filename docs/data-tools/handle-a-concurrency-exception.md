---
title: 处理并发异常
description: " (处理 System.Data.DBConcurrencyException) 的并发异常，当两个用户尝试同时更改数据库中的相同数据时，将引发此异常。"
...
当两个用户 () 更改数据库中的相同数据时，将引发并发 <xref:System.Data.DBConcurrencyException?displayProperty=fullName> 异常。 在此演练中，你将创建一Windows应用程序，用于演示如何捕获 、查找导致错误的行，以及了解如何处理 <xref:System.Data.DBConcurrencyException> 错误的策略。
...
3. 使用 创建一个窗体 <xref:System.Windows.Forms.DataGridView> 以显示数据。

4. 使用 Northwind 数据库中的 Customers 表中的数据填充数据集。

5. 使用 **中"显示** 表数据 **服务器资源管理器** 访问 Customers-table 的数据并更改记录。

6. 将同一记录更改为其他值，更新数据集，并尝试将更改写入数据库，这导致引发并发错误。

7. 捕获错误，然后显示记录的不同版本，允许用户确定是继续更新数据库还是取消更新。
...
4. 将项目 **命名 ConcurrencyWalkthrough**，然后选择"确定 **"。**

     **ConcurrencyWalkthrough** 项目已创建并添加到 **解决方案资源管理器，并且** 会在设计器中打开一个新窗体。

## <a name="create-the-northwind-dataset"></a>创建 Northwind 数据集

接下来，创建名为 **NorthwindDataSet 的数据集**：

1. 在"**数据"菜单** 上，选择 **"添加新数据源"。**
...
3. 从可用连接列表中选择与 Northwind 示例数据库的连接。 如果连接列表中没有可用的连接，请选择"新建 **连接"。**

    > [!NOTE]
    > 如果要连接到本地数据库文件，请在系统询问是否将该文件添加到项目时选择"否"。

4. 在"**将连接字符串保存到应用程序配置文件"屏幕上**，选择"下一 **步"。**
...
在本部分，你将通过将"数据源"窗口中的"客户"项拖动到"Windows <xref:System.Windows.Forms.DataGridView?displayProperty=nameWithType> 窗体"来创建。 

1. 若要打开"**数据源"** 窗口，请选择"数据 **"** 菜单上的"**显示数据源"。**
...
4. 将表拖到窗体的空白区域。
     ...
     名为 <xref:System.Windows.Forms.DataGridView> **CustomersDataGridView** 的控件和名为 <xref:System.Windows.Forms.BindingNavigator> **CustomersBindingNavigator** 的 控件将添加到绑定到 的窗体 <xref:System.Windows.Forms.BindingSource> 中。 这反过来又绑定到 NorthwindDataSet 中的 Customers 表。

## <a name="test-the-form"></a>测试窗体

现在可以测试窗体，以确保它的行为与预期一样，到此时：

1. 选择 **F5** 以运行应用程序。

     窗体上会显示 <xref:System.Windows.Forms.DataGridView> 一个控件，该控件用 Customers 表中的数据填充。

2. 在“调试”菜单中，选择“停止调试”。

## <a name="handle-concurrency-errors"></a>处理并发错误

如何处理错误取决于控制应用程序的特定业务规则。 对于本演练，我们将使用以下策略作为如何处理并发错误的示例。
...
### <a name="to-enable-the-handling-of-concurrency-errors"></a>启用并发错误的处理
...
尝试执行更新并引发异常时，通常需要对所引发异常提供的信息执行某些操作。 在本部分，你将添加尝试更新数据库的代码。 还可以处理 <xref:System.Data.DBConcurrencyException> 可能会引发的任何异常以及任何其他异常。

> [!NOTE]
> 稍后 `CreateMessage` `ProcessDialogResults` 在演练中添加 和 方法。

1. 在 方法下面添加以下 `Form1_Load` 代码：
...
2. 替换 `CustomersBindingNavigatorSaveItem_Click` 方法以调用 `UpdateDatabase` 方法，如下所示：
...
刚编写的代码调用 `CreateMessage` 过程，以向用户显示错误信息。 对于本演练，请使用消息框向用户显示记录的不同版本。 这使用户可以选择是使用更改覆盖记录还是取消编辑。 在用户选择一个选项 (单击消息框上的) 按钮后，响应将传递到 `ProcessDialogResult` 方法。

通过将以下代码添加到代码编辑器 来创建 **消息**。 在 方法下面输入 `UpdateDatabase` 以下代码：
...
还需要代码处理用户对消息框的响应。 选项是使用建议的更改覆盖数据库中的当前记录，或者放弃本地更改，然后使用数据库中当前的记录刷新数据表。 如果用户选择"是 **"，** 则 <xref:System.Data.DataTable.Merge%2A> 调用 方法，同时 *preserveChanges* 参数设置为 **true。** 这会导致更新尝试成功，因为记录的原始版本现在与数据库中的记录匹配。
...
现在可以测试窗体，以确保其行为与预期一样。 若要模拟并发冲突，在填充 NorthwindDataSet 后更改数据库中的数据。

1. 选择 **F5** 以运行应用程序。

2. 窗体出现后，保持其运行状态并切换到 Visual Studio IDE。

3. 在“视图”菜单中，选择“服务器资源管理器”。

4. 在 **服务器资源管理器** 中，展开应用程序使用的连接，然后展开"表 **"** 节点。

5. 右键单击 **"Customers"** 表，然后选择"**显示表数据"。**

6. 在 **ALFKI** (的第) ，将 **ContactName** 更改为 Maria **Anders2**。
...
8. 在 **ALFKI** (的第一条) ，将 **ContactName** 更改为 **Maria Anders1**。

9. 选择“保存”按钮。

     引发并发错误，并出现消息框。

   选择 **"** 否"将取消更新，并且使用数据库中当前的值更新数据集。 选择 **"** 是"会将建议的值写入数据库。

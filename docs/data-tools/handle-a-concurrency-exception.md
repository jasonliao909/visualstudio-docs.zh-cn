...
当两个用户 () 更改数据库中的相同数据时，将引发并发 <xref:System.Data.DBConcurrencyException?displayProperty=fullName> 异常。 本演练将创建一Windows应用程序，该应用程序演示如何捕获 、查找导致错误的行，并了解如何处理 <xref:System.Data.DBConcurrencyException> 错误的策略。
...
5. 使用 **服务器资源管理器** 中的"显示表数据"功能访问 Customers-table 的数据并更改记录。
...
刚编写的代码调用 `CreateMessage` 过程，以向用户显示错误信息。 对于本演练，请使用消息框向用户显示记录的不同版本。 这使用户可以选择是使用更改覆盖记录还是取消编辑。 在用户选择一个选项 (单击消息框上的) 按钮后，响应将传递给 `ProcessDialogResult` 方法。

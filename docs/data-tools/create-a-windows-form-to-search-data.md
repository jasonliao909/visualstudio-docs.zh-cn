...
description: 阅读如何创建 Windows 窗体以搜索数据的示例。 创建 Windows 窗体应用程序、数据源和窗体。 添加参数化。 测试应用。
...
可以使用"搜索条件生成器"对话框，将参数化查询添加到任何 TableAdapter (控件，以接受参数值并执行查询) **查询** 。 通过在“数据”菜单上（或任何 TableAdapter 智能标记上）选择“添加查询”命令来打开对话框。

本演练涉及以下任务：

- 使用数据源配置向导在应用程序中创建 **和配置** 数据源。
...
必须安装 **数据存储和处理** 工作负载。 请参阅[修改 Visual Studio](../install/modify-visual-studio.md)。

本演练使用 LocalDB SQL Server Express Northwind 示例数据库。

1. 如果没有 LocalDB SQL Server Express，请从下载页SQL Server Express安装它，或者通过 [](https://www.microsoft.com/sql-server/sql-server-editions-express)**Visual Studio 安装程序。** 在 **Visual Studio 安装程序** 中，可以将 SQL Server Express LocalDB 作为数据存储和处理工作负荷的一部分安装，也可以作为单个组件安装。

2. 按照以下步骤安装 Northwind 示例数据库：

    1. 在Visual Studio中， **打开SQL Server 对象资源管理器窗口** 。  (SQL Server 对象资源管理器作为数据存储和处理工作负荷的一部分安装在Visual Studio 安装程序 **.)** 展开 **SQL Server节点。** 右键单击 LocalDB 实例，然后选择"新建 **查询"。**

       查询编辑器窗口随即打开。

    2. 将 [Northwind Transact-SQL 脚本](https://github.com/MicrosoftDocs/visualstudio-docs/blob/master/docs/data-tools/samples/northwind.sql?raw=true) 复制到剪贴板。 此 T-SQL 脚本从头开始创建 Northwind 数据库，并使用数据填充该数据库。

    3. 将 T-SQL 脚本粘贴到查询编辑器中，然后选择"执行 **"** 按钮。

       短时间后，查询完成运行，并创建 Northwind 数据库。

## <a name="create-the-windows-forms-application"></a>创建Windows 窗体应用程序

:::moniker range="vs-2017"

为 **C# Windows 窗体创建** 一个新的 A0.NET Framework) Visual Basic 项目。 将该项目命名为 **WindowsSearchForm**。
...
1. 若要打开"**数据源"** 窗口，请使用"快速 (**Ctrl** + **Q**) ，然后搜索 **"数据源"。**

1. 在" **数据源"** 窗口中， **选择"添加新数据源** "以启动 **"数据源配置"** 向导。

1. 在 **“选择数据源类型”** 页上选择 **“数据库”** ，然后单击 **“下一步”**。

1. 在"**选择数据库模型"屏幕上**，选择"**数据集"，** 然后单击"下一 **步"。**
...
1. 在"**将连接字符串保存到应用程序配置文件"页上**，单击"下一 **步"。**
...
1. 确保Windows 窗体设计器具有活动焦点， **并且"数据源** "窗口已打开并固定。
...
## <a name="add-parameterization-search-functionality-to-the-query"></a>向查询 (参数化) 搜索功能

可以使用"搜索条件生成器"对话框将 WHERE 子句添加到 **原始** 查询：

1. 在窗体的设计图面正下方，选择 **"customersTableAdapter"** 按钮，然后在"属性 **"** 窗口中，选择"**添加查询..."。**

2. 在"搜索条件生成器"对话框的"**新建查询** 名称"区域中键入 **FillByCity。**
...
3. 将 `WHERE City = @City` 添加到“查询文本”区域的查询中。
    ...
    > [!NOTE]
    > 访问OLE DB数据源使用问号 ("？") 表示参数，因此 WHERE 子句将如下所示 `WHERE City = ?` ：。
...
运行应用程序会打开窗体，并准备好使用 参数作为输入：

1. 按 **F5** 运行该应用程序。

2. 在“City”文本框中键入“London”，然后单击“FillByCity”。

     数据网格中填充了符合条件的客户。 在此示例中，数据网格只显示其“City”列中有“London”值的客户。

...
description: 阅读一个示例，了解如何创建一个Windows表单来搜索数据。 创建Windows窗体应用程序、数据源和窗体。 添加参数化。 测试应用。
...
为 **C# Windows创建一 (.NET Framework)** 窗体应用Visual Basic。 将该项目命名为 **WindowsSearchForm**。
...
1. 请确保"Windows设计器具有活动焦点，**并且"数据源**"窗口已打开并固定。

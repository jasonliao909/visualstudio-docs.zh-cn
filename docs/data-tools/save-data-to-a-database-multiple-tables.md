---
title: 将数据保存到数据库（多个表）
description: 本演练使用数据集中的 DataSet 工具将数据从多个表保存到Visual Studio。
...
通过调用 TableAdapter 的 `Update` 方法，可以将应用程序中的数据保存回数据库。 将表从"数据源 **"** 窗口拖动到窗体上时，会自动添加保存数据所需的代码。 添加到窗体的其他任何表都需要手动添加此代码。 本演练显示了如何添加从多个表保存更新的代码。

本演练涉及以下任务：

- 使用数据源配置向导 在应用程序中创建 [和配置数据源](../data-tools/media/data-source-configuration-wizard.png)。

- 设置"数据源"窗口中 [项的控件](add-new-data-sources.md#data-sources-window)。 有关详细信息，请参阅设置从"数据源"窗口拖动时 [要创建的控件](../data-tools/set-the-control-to-be-created-when-dragging-from-the-data-sources-window.md)。
...
为 **C# Windows创建一** 个新的窗体应用Visual Basic。 将项目命名为 **UpdateMultipleTablesWalkthrough**。
...
6. 在"**将连接字符串保存到应用程序配置文件"中，选择**"下一 **步"。**
...
对于本演练，表中的数据位于"详细信息"布局中， `Customers` 其中数据显示在单个控件中。 表中的数据位于 `Orders` 控件中显示的 **网格** <xref:System.Windows.Forms.DataGridView> 布局中。

### <a name="to-set-the-drop-type-for-the-items-in-the-data-sources-window"></a>设置“数据源”窗口中项的拖放类型

1. 在" **数据源"窗口中** ，展开" **客户"** 节点。

2. 在"**客户"** 节点上，从控件列表中选择"详细信息"，将 **"Customers"表的** 控件更改为单个控件。 有关详细信息，请参阅设置从"数据源"窗口拖动时 [要创建的控件](../data-tools/set-the-control-to-be-created-when-dragging-from-the-data-sources-window.md)。
...
2. 将相关的“Orders”节点从“数据源”窗口拖到“Form1”上。
    ...
    > 相关的“Orders”节点位于“Fax”列下，该节点是“Customers”节点的子节点。
     ...
     用于导航记录的 <xref:System.Windows.Forms.DataGridView> 控件和工具栏（<xref:System.Windows.Forms.BindingNavigator>）将显示在窗体上。 和 `OrdersTableAdapter` <xref:System.Windows.Forms.BindingSource> 显示在组件栏中。

## <a name="add-code-to-update-the-database"></a>添加代码以更新数据库

通过调用“Customers”和“Orders”TableAdapter 的 `Update` 方法，可更新数据库。 默认情况下， 的"保存 **"** 按钮的事件处理程序将添加到窗体的代码，以 <xref:System.Windows.Forms.BindingNavigator> 将更新发送到数据库。 此过程修改代码以按正确的顺序发送更新。这消除了引发引用完整性错误的可能性。 该代码还将通过在 try-catch 块中包装更新调用来实现错误处理。 可以根据应用程序的需要修改代码。

> [!NOTE]
> 为清楚起见，本演练不使用事务。 但是，如果要更新两个或多个相关表，请包含事务内的所有更新逻辑。 事务是一个过程，可确保在提交任何更改之前对数据库进行的所有相关更改都成功。 有关详细信息，请参阅事务 [和并发](/dotnet/framework/data/adonet/transactions-and-concurrency)。

### <a name="to-add-update-logic-to-the-application"></a>将更新逻辑添加到应用程序

1. 选择 上的 **"保存** "按钮 <xref:System.Windows.Forms.BindingNavigator> 。 这会打开事件处理程序的代码 `bindingNavigatorSaveItem_Click` 编辑器。

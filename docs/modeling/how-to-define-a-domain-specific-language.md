...
6. **将属性映射到文本修饰器。**
   ...
   2. 在 **"DSL 详细信息"** 窗口中，单击"**修饰器映射"** 选项卡。如果未看到 **"DSL** 详细信息"窗口，请在"视图"菜单上指向"其他 **窗口**"，然后单击 **"DSL 详细信息"。** 通常需要提升此窗口的顶部以查看它的所有内容。
...
7. **生成用于创建域类的元素的工具箱项。**
   ...
   5. 将 **"工具箱** 图标"设置为将出现在工具箱中的图标。 可以将它设置为新图标或已用于其他工具的图标。
        ...
        返回到 DSL 定义关系图，选择工具，然后属性窗口"工具箱"图标 中的 **"下** 一步 **"**。 在" **选择位图** "对话框中，从.BMP菜单中选择你的位图文件。
...
5. **验证是否显示每个文本修饰器，** 以及：
   ...
   在首次测试形状后，你可能想要调整它的某些属性并添加一些更高级的功能。 有关详细信息，请参阅 [自定义和扩展Domain-Specific语言](../modeling/customizing-and-extending-a-domain-specific-language.md)。
...
### <a name="define-a-connector-to-display-the-relationship"></a>定义连接符以显示关系
 ...
 选中关系图元素映射后，打开 **"DSL 详细信息"** 窗口，然后打开" **修饰器映射"** 选项卡。

 选择每个 **修饰器，** 将 **Display 属性** 设置为正确的域属性。

 确保修饰器列表中的每个项旁边都显示 **一个选中** 标记。
...
| 不会显示 DSL 的工具箱，但并未更改程序集名称。<br /><br /> 或者，显示一个报告加载扩展失败的消息框。 | 重置实验实例，并重新生成解决方案。<br /><br /> 1.在 Windows “开始”菜单，在"所有程序"下展开 ，然后展开"工具 [!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)] "，然后单击"重置Microsoft Visual Studio **实例"。** <br />2.在"生成 **"菜单** 上，单击"**重新生成解决方案"。** |

---
title: 如何定义域特定语言
description: 了解如何从模板创建Visual Studio解决方案，以在 DSL (域) 。
...
ms.locfileid: "121386327"
---
# <a name="how-to-define-a-domain-specific-language"></a>如何定义域特定语言
若要在 DSL (定义) 特定) ，请从模板Visual Studio解决方案。 解决方案的重要组成部分是 DSL 定义关系图，它存储在 DslDefinition.dsl 中。 DSL 定义将定义 DSL 的类和形状。 在进行修改并将所做修改添加到这些元素后，可以添加程序代码以更详细地自定义 DSL。
...
- Visual Studio
- Visual Studio扩展开发 (包括 Visual Studio SDK) 
...
### <a name="test-the-solution"></a>测试解决方案
 ...
 如果运行从最小语言模板创建的解决方案，则试验Visual Studio示例类似：
...
- 根类显示在 DSL 定义关系图的左上角的"类 **和关系"下**。 将它重命名为与 DSL 不同的名称。 例如，名为 **MusicLibrary** 的 DSL 可能有名为 音乐 的 **根类**。

- 该关系图类显示在 DSL 定义关系图右下角的" **关系图元素"** 列中。 可能必须滚动到右侧才能看到它。 它通常命名为 _YourDsl_**Diagram**。

- 如果使用了"任务 **Flow** 模板，并且想要创建具有泳道的关系图，请保留并重命名 Actor 域类和 ActorSwimlane 形状。
...
## <a name="defining-domain-classes"></a><a name="classes"></a> 定义域类
 ...
 若要创建域类，可以从命名 **域类** 工具拖动到关系图，然后重命名该类。
...
|不直接显示。|由类作为一部分所组成的类。||
 ...
 若要同时创建域类及其嵌入，请单击" **嵌入** 关系"工具，然后单击父类，然后单击关系图的空白部分。

 通常不必调整嵌入关系及其角色的名称，因为它们将自动跟踪类名。

 有关详细信息，请参阅域[关系的属性和](../modeling/properties-of-domain-relationships.md)[域角色的属性](../modeling/properties-of-domain-roles.md)。
...
### <a name="add-domain-properties-to-each-domain-class"></a>将域属性添加到每个域类
 ...
 单击 **类中的** "域属性"，按 ENTER 键，然后键入属性的名称。 域属性的默认类型是 String。 如果要更改类型，请选择域属性，然后设置"属性" **窗口中** 的" **类型** "。 如果不需要的类型不在下拉列表中，请参阅添加 [属性类型](#addTypes)。

 **设置元素名称属性。** 在语言资源管理器中选择可以用于标识元素的域属性。 例如，在 Song 域类中可以选择 Title 域属性。 在" **属性"** 窗口中，将 **"Is Element Name"** 设置为 `true` 。
...
### <a name="create-derived-domain-classes"></a>创建派生的域类
 ...
 请考虑将 **基类的继承** 修饰符设置为 **抽象**。 如果你认为可能需要基类的实例，则请考虑为它们创建单独的派生类。

 派生类继承其基类的属性和角色。

### <a name="tidy-the-dsl-definition-diagram"></a>整理 DSL 定义关系图
 在添加关系时，某些类将显示在多个地方。 若要减少外观数量并扩大关系图，请右键单击关系的目标类，然后单击"**将树引入此处"。** 对于相反的效果，右键单击关系的目标类，然后单击"拆分 **树"。** 如果未看见这些菜单命令，则确保只选中域类。
...
1. **单击设计器工具栏** 中的"转换所有模板解决方案资源管理器生成 DSL 设计器代码。 可以自动化执行此步骤。 有关详细信息，请参阅 [如何自动转换所有模板](/previous-versions/visualstudio/visual-studio-2012/ff521399\(v\=vs.110\))。

2. **生成并运行 DSL。** 按 F5 或 CTRL+F5，以试验Visual Studio运行新实例。 在 VISUAL STUDIO实例中，打开或创建一个扩展名为 DSL 的文件。

3. **打开资源管理器。** 关系图的一侧是语言资源管理器窗口，通常命名为 *YourLanguage* Explorer。 如果未看见此窗口，则它可能位于“解决方案资源管理器”下方的选项卡上。 如果找不到它，请在"视图"菜单上指向"其他Windows"，**然后单击***"YourLanguage* **资源管理器"。**

     资源管理器将呈现模型的树视图。

4. **创建新元素。** 右键单击顶部的根节点，然后单击"**添加新**_YourClass"。_

     类的新实例显示在语言资源管理器中。

5. 在创建新实例时验证每个实例是否都具有不同名称。 只有在对域属性设置了 **Is 元素名称** 标志时，才发生这种情况。

6. **检查域属性。选择类的实例后，检查** 属性窗口。 它应显示在此域类上定义的域属性。

7. **保存文件，将其关闭，然后重新打开它**。 在展开节点后，你创建的所有实例都应在资源管理器中可见。

## <a name="defining-shapes-on-the-diagram"></a><a name="shapes"></a> 在关系图上定义形状
 可以将显示在关系图上的元素的类定义为矩形、椭圆或图标。

#### <a name="to-define-a-class-of-elements-that-appear-as-shapes-on-a-diagram"></a>定义显示为关系图上的形状的元素的类

1. **定义并测试域类，如定义**[域类 中所述](#classes)**。**  

   - 该类的父级应为根类。 即根类和新域类之间应存在嵌入关系。

   - 如果关系图具有泳道，则父级可以是映射到泳道的域类。 在继续此过程之前，请参阅[定义具有泳道 的 DSL。](#swimlanes)
...
3. **如果创建了图像形状 ，请定义图像**。
   ...
   5. 在"**选择图像**"对话框中，单击"文件名"下的下拉菜单，然后选择图像。

4. **将文本修饰器添加到形状，以显示域属性。**

    若要显示模型元素的名称或标题，将可能需要至少一个文本修饰器。

    右键单击形状类的标题，指向"**添加**"，然后单击"**文本修饰器"。** 设置修饰器的名称，在 属性窗口设置其 **位置**。

5. **连接关系图元素映射** 的每个形状映射到它应显示的域类。

    单击" **关系图元素映射"** 工具，然后单击域类，然后单击形状类。

6. **将属性映射到文本修饰器。**

   1. 选中域类和形状类之间表示关系图元素映射的灰色线。

   2. 在 **"DSL 详细信息"** 窗口中，**单击"修饰** 器地图选项卡。如果未看到 **"DSL** 详细信息"窗口，请在"视图"菜单上指向"其他Windows然后单击 **"DSL 详细信息****"。** 通常需要提升此窗口的顶部以查看它的所有内容。

   3. 选择修饰器的名称。 在 **"显示** 属性"下，选择域类的属性的名称。 为每个修饰器重复此过程。

       如果要显示相关元素的属性，请单击"路径"下的下拉树导航器 **以显示属性**。
...
7. **生成用于创建域类的元素的工具箱项。**
   ...
   2. 右键单击与 DSL **同名** 的"工具箱选项卡"下的节点，例如 MusicLibrary。 单击"**添加元素工具"。**

       > [!NOTE]
       > 如果右键单击"工具 **"** 节点，将看不到"**添加元素工具"。** 应改为单击其上方的节点。

   3. 在属性窗口元素工具的"类"中，将 **"** 类"设置为最近添加的域类。

   4. 设置 **标题** 和 **工具提示**。

   5. 将 **"工具箱** 图标"设置为将出现在工具箱中的图标。 可以将它设置为新图标或已用于其他工具的图标。

        若要创建新图标，请打开 中的 Dsl\Resources **解决方案资源管理器。** 复制并粘贴现有元素工具 BMP 文件之一。 重命名粘贴的副本，然后双击以对其进行编辑。

        返回到 DSL 定义关系图，选择该工具，然后属性窗口"工具箱"图标 中的 **"下** 一步 **"**。 在" **选择位图** "对话框中，从.BMP菜单中选择你的位图文件。

   有关详细信息，请参阅 Geometry[形状的属性和](../modeling/properties-of-geometry-shapes.md)[图像形状的属性](../modeling/properties-of-image-shapes.md)。
...
5. **验证是否显示每个文本修饰器，** 以及：
   ...
   在首次测试形状后，你可能想要调整它的某些属性并添加一些更高级的功能。 有关详细信息，请参阅 [自定义和扩展](../modeling/customizing-and-extending-a-domain-specific-language.md)Domain-Specific语言 。
 ...
 **根据需要调整每个角色** 的倍数。 如果希望每个 Person 最多只有一个 Manager，则将显示在关系图上的 Manager 标签下的重数设置为 0..1。

 **将域属性添加到关系。** 在该图中，Artist-Album 关系具有角色的属性。

 **如果同一个类在同一** 对模型元素之间可以存在多个链接，请设置关系的"允许重复项"属性。 例如，可以允许 Teacher 向相同的 Student 教授多个 Subject。
...
### <a name="define-a-connector-to-display-the-relationship"></a>定义连接符以显示关系
 ...
 将连接器 **工具** 拖到 DSL 定义关系图上。

 如果想要在连接符上显示标签，则添加文本修饰器。 设置其位置。 若要允许用户移动文本修饰器，请设置其 Is **Moveable** 属性。

 使用 **关系图元素映射** 工具将连接器链接到引用关系。

 选中关系图元素映射后，打开 **"DSL** 详细信息"窗口，然后打开"修饰器 **地图选项卡。**

 选择每个 **修饰器，** 将 **Display 属性** 设置为正确的域属性。

 请确保修饰器列表中的每个项旁边都显示 **一个选中** 标记。
...
- 单击 **"连接** 生成器"，然后选择新关系的适当生成器。

- 将 **"工具箱** 图标"设置为要显示在工具箱中的图标。 可以将它设置为新图标或已用于其他工具的图标。
...
2. **创建列表项域类**。 单击 " **嵌入关系** " 工具，单击 container 类 (唱片集) 然后单击关系图的空白部分。 这将创建在示例图中名为 Song 的域类。
...
| 不会显示 DSL 的工具箱，但并未更改程序集名称。<br /><br /> 或者，显示一个报告加载扩展失败的消息框。 | 重置实验实例，并重新生成解决方案。<br /><br /> 1.在Windows "开始"菜单"所有程序"下，展开 ，再展开"工具"，然后单击"重置Microsoft Visual Studio [!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)] **实例"。** <br />2.在"生成 **"菜单** 上，单击"**重新生成解决方案"。** |

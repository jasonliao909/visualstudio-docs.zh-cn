---
title: 如何定义域特定语言
description: 了解如何从模板创建 Visual Studio 解决方案来定义域特定语言 (DSL) 。
...
- Visual Studio
- Visual Studio 扩展开发工作负荷 (包括 Visual Studio SDK) 
- 建模 SDK (将其安装为 Visual Studio 中的单个组件) 
...
2. 单击每个模板以查看说明。 选择最近似你想要创建的解决方案。
   ...
   - 选择 "**任务 Flow** " 以创建具有泳道的 DSL。 泳道是关系图的垂直或水平分区。
   ...
   - 选择 "**最小 WinForm 设计器**" 或 "**最小 wpf 设计器**"，创建在 Windows 窗体或 WPF 表面上显示的 DSL。 必须编写代码，才能定义编辑器。 有关详细信息，请参阅以下主题：
...
### <a name="test-the-solution"></a>测试解决方案
 ...
 如果运行从 **最小语言** 模板创建的解决方案，则实验性 Visual Studio 将类似于以下示例：
...
- 根类出现在 DSL 定义关系图的左上角的 " **类和关系**" 下。 将它重命名为与 DSL 不同的名称。 例如，名为 **MusicLibrary** 的 DSL 可能具有名为 **音乐** 的根类。

- 关系图类出现在 DSL 定义关系图的右下方的 " **关系图元素** " 列中。 可能必须滚动到右侧才能看到它。 它通常名为 _yourdsl 可_**关系图**。

- 如果你使用了 **任务 Flow** 模板，并且想要使用泳道创建关系图，请保留并重命名执行组件域类和 ActorSwimlane 形状。
...
### <a name="define-a-connector-to-display-the-relationship"></a>定义连接符以显示关系
 ...
 选中 "关系图元素映射"，打开 " **DSL 详细信息**" 窗口，然后打开 "**修饰器地图**" 选项卡。
...
1. **创建容器域类**。 单击" **嵌入关系** "工具，单击模型的根类，然后单击 DSL 定义关系图的空白部分。 这将创建在示例图中名为 Album 的域类。
     ...
     将域属性（如 Name）添加到 类，并设置其 Is **元素** 名称标志属性窗口。

2. **创建列表项域类**。 单击" **嵌入关系** "工具，单击"Album (容器) ，然后单击关系图的空白部分。 这将创建在示例图中名为 Song 的域类。
...
6. 将 **"工具箱** 图标"设置为将出现在工具箱中的图标。 可以将它设置为新图标或已用于其他工具的图标。
     ...
     返回到 DSL 定义关系图，选择该工具，然后属性窗口"工具箱"图标 中的 **"下** 一步 **"**。 在" **选择位图** "对话框中，从下拉菜单中选择 BMP 文件。
...
1. **向隔离舱形状 添加隔离舱**。 在 DSL 定义关系图上，右键单击隔离舱形状类，指向"**添加**"，然后单击"隔离 **舱"。**

2. 将 **"显示的元素集合路径"** 设置为导航到链接，而不是其目标元素。 单击下拉菜单并使用树视图来选择引用关系而不是其目标。 在示例中，关系为 **ArtistAppearedOnAlbums**。

3. 将 **"路径"设置为** "显示属性"，以从链接导航到目标元素。 在示例中，这是 **Artist**。

4. 将 **"显示** 属性"设置为目标元素的适当属性，例如"**名称"。**
...
## <a name="defining-ports-on-the-boundary-of-another-shape"></a><a name="ports"></a> 在另一个形状的边界上定义端口
 ...
 若要查看使用端口的示例，请在创建新的 DSL解决方案时选择"组件关系图"模板。 此示例显示在定义端口时可以考虑的主要几点：

- 存在一个表示端口的容器的域类：`Component`。

- 存在一个表示端口的域类。 在本示例中，此项为 `ComponentPort`。

- 存在一个从容器域类到端口域类的嵌入关系。 有关详细信息，请参阅定义 [域类](#classes)。
...
- 端口域类可映射到端口形状。 可以将派生类映射到单独的端口形状类，或将基类映射到一个端口形状类。
  ...
  在其他方面，端口形状的行为如定义形状 [中所述](#shapes)。

  有关详细信息，请参阅端口 [形状的属性](../modeling/properties-of-port-shapes.md)。
...
### <a name="domain-enumerations-and-literals"></a>域枚举和文本
 ...
 若要添加域枚举，请在 DSL 资源管理器中右键单击模型的根 **，** 然后单击"**添加新域枚举"。** 元素将显示在 DSL 资源管理器 **中的"** 域类型 **"节点** 下。 此元素不会显示在关系图上。

 若要将枚举文本添加到域枚举，请在 **DSL** 资源管理器中右键单击域枚举，然后单击"添加新枚举 **文本"。**

 默认情况下，一次只能将具有枚举类型的属性设置为一个枚举值。 如果希望用户和程序员能够设置值的任意组合（"位字段"）-则设置枚举的 **IsFlags** 属性。

### <a name="external-types"></a>外部类型
 设置域属性的类型时，如果在"类型"下拉列表中找不到想要的类型，可以添加外部类型。  例如，可以将 **System.Drawing.Color** 类型添加到列表中。

 若要添加类型，请在 DSL 资源管理器中右键单击模型的根，然后单击"**添加新的外部类型"。** 在属性窗口中，将名称设置为 **Color，** 将命名空间设置为 **System.Drawing**。 此类型现在显示在 DSL 资源管理器中的"域 **类型"下**。 无论何时设置域属性的类型，你都可以选择此类型。
...
1. 使用 DSL 定义的更多功能微调 DSL。 例如，你可以生成可创建多种类型的连接符的单个连接符工具，也可以通过删除一个元素也将删除相关元素的方式来控制规则。 这些技术主要通过在 DSL 定义中设置值来实现，而有些技术则需要几行程序代码。
     ...
     有关详细信息，请参阅 [自定义和扩展Domain-Specific语言](../modeling/customizing-and-extending-a-domain-specific-language.md)。

2. 通过使用程序代码扩展模型工具以实现更高级的效果。 例如，你可以创建可更改模型的菜单命令，也可以创建集成两个或多个 DSL 的工具。 VMSDK 专门用于轻松将扩展和从 DSL 定义生成的代码相集成。  有关详细信息，请参阅 [编写代码以自定义Domain-Specific语言](../modeling/writing-code-to-customise-a-domain-specific-language.md)。
...
### <a name="changing-the-dsl-definition"></a>更改 DSL 定义
 ...
 另 [请参阅如何：更改语言 Domain-Specific命名空间](../modeling/how-to-change-the-namespace-of-a-domain-specific-language.md)。

## <a name="troubleshooting"></a><a name="trouble"></a> 故障排除
 下表列出了在设计 DSL 时遇到的一些最常见问题，以及其解决方案的建议。 可视化工具扩展性 [论坛中提供了更多建议](https://social.msdn.microsoft.com/Forums/vstudio/en-US/home?forum=dslvsarchx)。

| 问题 | 建议 |
|-|-|
| 在 DSL 定义文件中进行的更改不起作用。 | 单击 **上述工具栏中的** "转换所有模板解决方案资源管理器，然后重新生成解决方案。 |
| 形状显示了修饰器的名称而不是属性值。 | 设置修饰器映射。 在 DSL 定义关系图上，单击“关系图元素映射”，它是域类和形状类之间的灰色线条。<br /><br /> 打开 **"DSL 详细信息"** 窗口。 如果看不到它，请在"视图"菜单上指向"其他Windows"，**然后单击**"DSL 详细信息 **"。**<br /><br /> 单击"**修饰器地图** 选项卡。选择修饰器的名称。 确保选中它旁边的框。 在 **"显示属性**"下，选择域属性的名称。<br /><br /> 有关详细信息，请参阅 [关系图上的形状](#shapes)。 |
...
| 不会显示 DSL 的工具箱，但并未更改程序集名称。<br /><br /> 或者，显示一个报告加载扩展失败的消息框。 | 重置实验实例，并重新生成解决方案。<br /><br /> 1. 在 Windows "开始"菜单上的 "**所有程序**" 下，依次展开 [!INCLUDE[vssdk_current_long](../misc/includes/vssdk_current_long_md.md)] "工具" 和 "**工具**"，然后单击 **"重置 Microsoft Visual Studio 实验实例"**。<br />2. 在 " **生成** " 菜单上，单击 " **重新生成解决方案**"。 |

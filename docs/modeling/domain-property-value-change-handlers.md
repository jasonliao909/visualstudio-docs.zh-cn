...
域特定语言的每个域属性都由嵌入在其父域类内的类处理。 它的名称遵循“PropertyNamePropertyHandler”格式。 可以在 Dsl\Generated Code\DomainClasses.cs 文件中检查此属性处理程序类。 在该类中，在值发生更改前立即调用 `OnValueChanging()`，在值发生更改后立即调用 `OnValueChanged()`。
...
- 无法将更改处理程序添加到表示关系的角色的属性。 相反，可在关系类上定义 `AddRule` 和 `DeleteRule`。 当创建或更改链接时，将触发这些规则。 有关详细信息，请参阅[模型中的规则传播更改](../modeling/rules-propagate-changes-within-the-model.md)。
...
如果定义 ChangeRule，则它将在事务（属性的值在其中发生更改）的末尾执行。  有关详细信息，请参阅[模型中的规则传播更改](../modeling/rules-propagate-changes-within-the-model.md)。

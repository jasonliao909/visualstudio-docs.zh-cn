---
title: 自定义工具和工具箱
description: 了解必须如何为想要使用户添加到其模型的元素定义工具箱项。
...
5. 若要测试 DSL，请按 F5 或 CTRL+F5，并在 Visual Studio 的实验实例中，打开示例模型文件。 新工具应显示在工具箱上。 将它拖动到关系图上以验证它是否将创建新元素。
     ...
     如果该工具未显示，请停止该实验 Visual Studio。 在 Windows“开始”菜单中，运行“重置 Microsoft Visual Studio 2010 实验实例”。 在“生成”菜单上，单击“重新生成解决方案” 。 然后，再次测试 DSL。
...
- 在其他类上定义元素合并指令，从而允许它们接受此类的新实例，并允许它们在创建新元素后创建其他链接。 例如，你可以允许用户将“注释”放到其他元素上，从而在两个元素之间创建引用链接。
     ...
     有关详细信息，请参阅[自定义元素的创建和移动](../modeling/customizing-element-creation-and-movement.md)。

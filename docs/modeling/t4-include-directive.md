...
- `filePath` 可以是绝对的，也可以相对于当前模板文件。
   ...
   此外，特定Visual Studio扩展可以指定自己的目录来搜索包含文件。 例如，在 DSL 工具 (可视化和建模 SDK) ，以下文件夹将添加到 include 列表中 `Program Files\Microsoft Visual Studio 10.0\Common7\IDE\Extensions\Microsoft\DSL SDK\DSL Designer\11.0\TextTemplates` ：。

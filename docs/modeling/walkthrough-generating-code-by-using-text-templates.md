---
title: 演练：使用文本模板生成代码
description: 了解代码生成允许你生成强类型化的程序代码，但可以在源模型更改时轻松进行更改。
...
在强类型版本中，对 XML 架构的更改会导致类发生更改。 编译器突出显示必须更改的应用程序代码部分。 在使用泛型 XML 代码的非类型化版本中，没有此类支持。
...
1. 在 **解决方案资源管理器** 中，右键单击项目，单击"**添加**"，然后单击"新建 **项"。**
...
3. 在文件的模板指令中，将 `hostspecific` 属性更改为 `true`。
     ...
     此更改将使模板代码能够访问 Visual Studio 服务。
...
设置 `hostspecific` 指令的 `<#@template#>` 属性允许模板获取对 Visual Studio API 的访问权限。 模板可通过该步骤获取项目文件的位置，避免在模板代码中使用绝对文件路径。

---
title: 使用 MEF 扩展 DSL
description: 了解如何使用 Managed Extensibility Framework (MEF) 扩展域特定语言 (DSL)。
...
可以使用 Managed Extensibility Framework (MEF) 扩展域特定语言 (DSL)。 你或其他开发人员将能够为 DSL 编写扩展，而无需更改 DSL 定义和程序代码。 此类扩展包括菜单命令、拖放处理程序和验证。 用户将能够安装 DSL，然后可选择为其安装扩展。
...
1. 在 DslPackage 项目中创建一个名为 MefExtension 的新文件夹。 向其中添加以下文件：

     文件名：`CommandExtensionVSCT.tt`

    > [!IMPORTANT]
    > 将此文件中的 GUID 设置为与 DslPackage\GeneratedCode\Constants.tt 中定义的 GUID CommandSetId 相同
    ...
    如果添加此文件，则必须使用 DSL 资源管理器中 EditorValidation 中的至少一个开关在 DSL 中启用验证。
...
2. 在 Dsl 项目中创建一个名为 MefExtension 的新文件夹。 向其中添加以下文件：
...
6. 在“属性”窗口中，请确保至少一个名为 Uses 的属性为 `true`。

7. 在“解决方案资源管理器”工具栏中，单击“转换所有模板”。

     附属文件显示在添加的每个文件下方。

8. 生成并运行解决方案以验证其是否仍在工作。

DSL 现已启用 MEF。 可以将菜单命令、笔势处理程序和验证约束编写为 MEF 扩展。 可以将这些扩展与其他自定义代码一起写入 DSL 解决方案中。 此外，你或其他开发人员可以编写单独的 Visual Studio 扩展来扩展 DSL。

## <a name="create-an-extension-for-a-mef-enabled-dsl"></a>为启用了 MEF 的 DSL 创建扩展

如果你有权访问自己或其他人创建的启用了 MEF 的 DSL，你可以为它编写扩展。 扩展可用于添加菜单命令、笔势处理程序或验证约束。 若要创作这些扩展，请使用 Visual Studio 扩展 (VSIX) 解决方案。 该解决方案包含两个部分：一个生成代码程序集的类库项目和一个打包程序集的 VSIX 项目。
...
2. 在新项目中，添加对 DSL 程序集的引用。
   ...
   - 如果你有权访问 DSL 项目，则可以在目录 Dsl\\bin\\\* 下找到程序集文件

   - 如果你有权访问 DSL VSIX 文件，则可以通过将 VSIX 文件的文件扩展名更改为“.zip”来查找程序集。 解压缩.zip文件。
...
7. 单击“添加内容”。 在对话框中，将“内容类型”设置为“MEF 组件”，将“源项目”设置为你的类库项目  。
...
8. 向 DSL 添加 VSIX 引用。
   ...
   2. 在对话框中，单击“添加有效负载”，然后找到 DSL 的 VSIX 文件。 VSIX 文件在 DSL 解决方案中的 DslPackage\\bin\\\* 中生成。
       ...
       这允许用户同时安装 DSL 和你的扩展。 如果用户已安装 DSL，则只会安装你的扩展。

9. 查看和更新 source.extension.vsixmanifest 的其他字段。 单击“选择版本”，并验证是否设置了正确的 Visual Studio 版本。
...
可以在单独的 DSL 扩展解决方案的程序集代码项目中编写扩展。 还可以在 DslPackage 项目中使用 MEF，作为 DSL 的一部分编写命令、笔势和验证代码的简便方法。

### <a name="menu-commands"></a>菜单命令

若要编写菜单命令，请定义一个实现 <xref:Microsoft.VisualStudio.Modeling.ExtensionEnablement.ICommandExtension> 的类，并使用 DSL 中定义的特性作为该类的前缀，名为 YourDsl`CommandExtension`。 可以编写多个菜单命令类。

每当用户右键单击关系图时，都会调用 `QueryStatus()`。 它应检查当前选择并设置 `command.Enabled` 以指示命令何时适用。
...
验证方法由 DSL 生成的 `ValidationExtension` 特性以及由 <xref:Microsoft.VisualStudio.Modeling.Validation.ValidationMethodAttribute> 标记。 方法可以出现在任何未由特性标记的类中。

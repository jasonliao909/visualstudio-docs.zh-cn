---
title: 在你的开发过程中使用模型
description: 请注意，在 Visual Studio 中，可以使用模型来帮助你了解和更改系统、应用程序或组件。
...
- 有时可使用模型生成代码或其他项目（如数据库架构或文档）。 例如，从模型生成 Visual Studio 的建模组件。 有关详细信息，请参阅 [从模型生成和配置应用](../modeling/generate-and-configure-your-app-from-models.md)。
...
若要详细了解此技术，请参阅 [从模型开发测试](../modeling/develop-tests-from-a-model.md)。
...
可以通过多种类型的关系图中查看模型。 有关模型和关系图的信息，请参阅 [为应用创建模型](../modeling/create-models-for-your-app.md)。
...
|用户需求<br /><br /> 用户对系统的需求的定义。|- 可以在单独的文档中描述业务规则和服务要求的质量。|
|高级设计<br /><br /> 系统的整体结构：主要组件以及它们如何结合在一起。|- 依赖项关系图描述如何将系统构建为相互依赖的部分。 可以针对依赖项关系图验证程序代码，以确保它符合体系结构。|
|代码分析<br /><br /> 可以从代码生成关系图。|- 依赖项关系图显示类之间的依赖关系。 可以针对依赖项关系图验证更新的代码。<br />- 类图显示代码中的类。|

...
4. 在 **"添加到建模Project"** 中，浏览到并选择解决方案中的现有建模项目。
     ...
     选择 **"创建新的建模项目"，** 将新的建模项目添加到解决方案。

    > [!NOTE]
    > 依赖项关系图必须存在于建模项目中。 但是，你可以将其链接到解决方案中任何位置的项。

5. 请确保保存建模项目和依赖项关系图。

## <a name="drag-and-drop-or-copy-and-paste-from-a-code-map"></a>从代码图拖放或复制和粘贴

1. 使用"体系结构"菜单为解决方案生成 **代码** 图。

2. 如果只想在产品代码中强制执行依赖项，请考虑应用代码图筛选器来删除解决方案文件夹和"测试资产"。

3. 在生成的代码图上，删除"外部"节点，或展开该节点以显示外部程序集，具体取决于是否要强制实施命名空间依赖项，并从代码映射中删除不需要的程序集。
...
7. 这会显示当前应用体系结构。 确定希望体系结构是什么，并相应地修改依赖项关系图。

![从代码图生成的依赖项关系图](media/dependency-validation-01.png)

## <a name="create-layers-from-artifacts"></a><a name="CreateLayers"></a> 从项目创建层
...
|向层中添加项目|将项目拖到层上。|
|创建新的未链接的层|在" **工具箱"** 中，展开 **"依赖项关系图** "部分， **然后将"层** "拖动到依赖项关系图。<br /><br /> 若要添加多个层，请双击该工具。 完成后，选择指针 **工具** 或按 **ESC** 键。<br /><br /> - 或 -<br /><br /> 打开依赖项关系图的快捷菜单，选择"**添加"，** 然后选择"层 **"。**|
|创建嵌套的层|将现有层拖到另一个层上。<br /><br /> - 或 -<br /><br /> 打开层的快捷菜单，**选择"添加**"，然后选择"层 **"。**|
|创建包含两个或更多现有层的新层|选择层，打开所选快捷菜单，然后选择"分组 **"。**|
|更改层的颜色|将 **"Color"** 属性设置为想要的颜色。|

...
在代码图的工具栏上，依次选择“布局” > “分析器”，然后选择要运行的分析器 ：

|**Analyzer**|**标识节点**|
|-|-|
|**循环引用分析器**|每个其他节点上也具有循环依赖项。 备注：展开“泛型”组时，代码图上不显示该组中存在的循环依赖项 。|
|**查找中心分析器**|位于前 25% 个高度连接的节点当中<br /><br /> **若要隐藏代码图上的所有其他节点**<br /><br /> 打开代码图的快捷菜单，依次选择“高级”、“选择”、“隐藏未选内容”  。<br />     代码图会隐藏未选定的节点，并且分析器会将新节点标识为中心。|
|**未引用的节点分析器**|不具有来自任何其他节点的引用。 警告：在假设不使用代码之前，验证每种情况。 无法在代码中以静态方式找到某些依赖项，如 XAML 依赖项和运行时依赖项。|

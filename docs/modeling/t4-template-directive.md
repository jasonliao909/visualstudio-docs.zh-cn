---
title: T4 模板指令
description: 了解一Visual Studio T4 文本模板通常以模板指令开头，该指令指定应如何处理模板。
...
如果 `hostspecific` `true` 为 且你正在使用 Visual Studio，可以强制转换 `this.Host` 到 IServiceProvider 以访问Visual Studio功能。 还可以使用 `Host.ResolvePath(filename)` 来获得项目中文件的绝对路径。 例如：
...
`language`属性指定语句 ([!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] 表达式) [!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)] 源代码的语言或语言。 从中生成输出的中间代码文件将使用此语言。 此语言与您的模板生成的语言无关，它可以是任何类型的文本。
...
设计时文本模板是自定义工具设置为 **TextTemplatingFileGenerator 的文件**。  该模板会生成代码或文本的输出文件，该文件构成了项目Visual Studio部分。 若要生成输出文件，首先要将模板转换为中间程序代码文件，你通常不会看到该文件。 `inherits` 特性为此中间代码指定基类。

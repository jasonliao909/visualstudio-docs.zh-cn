...
由于重载只是字符串或参数的不同 <xref:System.Uri> ，因此假定该字符串表示 (URI) 的统一资源标识符。 URI 的字符串表示形式容易导致分析和编码错误，并且可造成安全漏洞。 <xref:System.Uri> 类以一种安全的方式提供这些服务。 为了获得类的优点 <xref:System.Uri> ，字符串重载应 <xref:System.Uri> 使用字符串参数调用重载。

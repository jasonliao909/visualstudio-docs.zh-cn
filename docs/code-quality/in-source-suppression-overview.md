...
ms.workload:
- multiple
ms.openlocfilehash: eba0c25c194d09c379ce2eeb672faeceeb421d2f
ms.sourcegitcommit: d9cab667735450e735622f8b93266f07b8046f3e
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 04/06/2022
ms.locfileid: "141402225"
---
# <a name="suppress-code-analysis-violations"></a>禁止显示代码分析违规情况

指示警告不适用通常很有用。 禁止显示代码分析冲突表示已查看代码的团队成员，并可以禁止显示警告。 以下部分介绍了使用 Visual Studio IDE 取消代码分析冲突的不同方法。
...
- 从“错误列表”中
  ...
  - 如果禁止 **在源中**，"**预览更改**"对话框将打开并显示 C# [#pragma警告](/dotnet/csharp/language-reference/preprocessor-directives/preprocessor-pragma-warning)的预览，或Visual Basic [#Disable警告](/dotnet/visual-basic/language-reference/directives/directives)指令添加到源代码中。
...
- 从“错误列表”中选择要禁止显示的规则，然后右键单击并选择“禁止显示” > “在禁止显示文件中”。 此时会打开 **"预览更改** "对话框，并显示 <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> 添加到全局抑制文件的属性预览。
...
- 从“分析”菜单中，在菜单栏上选择“分析” > “生成并禁止显示待处理的问题”，以禁止显示所有当前违规情况。 取消所有当前冲突有时称为"基线"。

::: moniker-end
::: moniker range="vs-2017"

- 从“分析”菜单中，在菜单栏上选择“分析” > “运行代码分析并禁止显示待处理的问题”，以禁止显示所有当前违规情况。 取消所有当前冲突有时称为"基线"。
::: moniker-end
...
该 <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> 属性是一个条件属性，包含在托管代码程序集的 IL 元数据中。 仅当编译时定义CODE_ANALYSIS编译符号时，才包含此属性。

在 C++/CLI 中，在头文件中使用宏 CA\_SUPPRESS\_MESSAGE 或 CA\_GLOBAL\_SUPPRESS_MESSAGE 来添加特性。

> [!NOTE]
> 不应在发布版本上使用源内禁止显示，以防止意外传送源内禁止显示元数据。
...
- **范围** - 要禁止显示警告的目标。 如果未指定目标，则设置为特性的目标。 支持 [的范围](xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute.Scope) 包括：

  - [`module`](#module-suppression-scope) - 此范围禁止显示针对程序集的警告。 它是应用于整个项目的全局禁止显示。

  - `resource` -（仅[旧版 FxCop](../code-quality/static-code-analysis-for-managed-code-overview.md)）此范围禁止显示在写入到属于模块（程序集）一部分的资源文件中的诊断信息的警告。 对于仅分析源文件的 Roslyn 分析器诊断，此范围在 C#/VB 编译器中不受读取或尊重。
...
代码分析警告在应用 <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> 特性的级别禁止显示。 例如，可以在程序集、模块、类型、成员或参数级别应用该特性。 应用此属性的目的是将抑制信息紧密耦合到发生冲突的代码。

禁止显示的一般形式包括规则类别和规则标识符，其中包含规则名称的可选人工可读取的表示形式。 例如：

`[SuppressMessage("Microsoft.Design", "CA1039:ListsAreStrongTyped")]`

如果出于严格的性能原因来最小化源内禁止显示元数据，可以省略规则名称。 规则类别及其规则 ID 共同构成足够唯一的规则标识符。 例如：
...
禁止显示特性可以应用于方法，但不能嵌入在方法主体中。 如果将属性添加到 <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> 方法，将禁止显示特定规则的所有冲突。

在某些情况下，你可能想要禁止显示冲突的特定实例。 请考虑将来的代码不会自动免除代码分析规则的示例。 某些代码分析规则允许使用 `MessageId` 属性 <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> 来禁止显示冲突的特定实例。 通常，针对特定符号（局部变量或参数）的违规情况的旧规则遵守 `MessageId` 属性。 [CA1500:VariableNamesShouldNotMatchFieldNames](../code-quality/ca1500.md) 就是此类规则的一个示例。 但是，针对可执行代码（非符号）的违规情况的旧规则不遵守 `MessageId` 属性。 此外，.NET Compiler Platform（“Roslyn”）分析器不遵守 `MessageId` 属性。
...
可以通过指定显式范围来表示任何禁止显示。 这些禁止显示必须位于全局级别。 不能通过修改类型来指定成员级抑制。
...
全局禁止显示文件保留全局级别的禁止显示或未指定目标的禁止显示。 例如，程序集级别的违规情况的禁止显示存储在此文件中。 此外，一些 ASP.NET 禁止显示存储在此文件中，因为项目级别的设置不适用于窗体后面的代码。 首次在"**错误列表**"窗口中选择 **"禁止** 显示"命令的"**Project抑制文件**"选项时，将创建全局抑制文件并将其添加到项目中。
...
托管代码编译器和一些第三方工具生成代码以帮助快速开发代码。 源文件中显示的编译器生成的代码使用 `GeneratedCodeAttribute` 属性进行标记。

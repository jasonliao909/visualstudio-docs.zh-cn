...
## <a name="content-type-mappings"></a>内容类型映射
 通过 HTTP 发布时，内容类型 (也称为 MIME 类型) *。应用程序* 文件应为 "应用程序/x 应用程序"。 如果在服务器上安装了 .NET Framework 2.0，将自动为您设置此设置。 如果未安装此功能，则需要为应用程序 vroot 或整个服务器 (MIME 类型 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)]) 。

 如果使用 IIS 服务器进行部署，请运行<em>inetmgr。</em>exe，为 .application 扩展添加新的内容类型"application/x-ms-application"。 

## <a name="http-compression-issues"></a>HTTP 压缩问题
 使用 ，可以执行使用 HTTP 压缩的下载，HTTP 压缩是一种 Web 服务器技术，在将流发送到客户端之前，它使用 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] GZIP 算法压缩数据流。 客户端（本例中为 ）在读取文件 [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] 之前解压缩流。

...
ms.locfileid: "122073872"
---
# <a name="deploy-com-components-with-clickonce"></a>使用 ClickOnce 部署 COM 组件
传统上，部署旧版 COM 组件是一项困难的任务。 组件需要全局注册，因此可能会导致重叠应用程序之间产生不良副作用。 这种情况通常不是应用程序.NET Framework问题，因为组件与应用程序完全隔离或并行兼容。 Visual Studio，可以在 WINDOWS XP 或更高版本的操作系统上部署独立的 COM 组件。
...
2. 在"**新建Project"** 对话框中，选择"Visual Basic"节点，然后选择一ActiveX **DLL** 项目。 在“名称”  框中键入 `VB6Hello`。
...
1. 使用Visual Basic，在"文件 **"菜单中**，单击"**新建****"，Project**。
...
9. 运行应用程序。 在“调试”菜单中，单击“开始调试” 。
   ...
   接下来，需要隔离控件。 应用程序使用的每个 COM 组件在项目中表示为 COM 引用。 这些引用显示在" **引用"窗口** 的"引用 **"解决方案资源管理器** 下。  (请注意，可以直接使用 Project 菜单上的"添加引用 **"** 命令添加引用，也可以将 ActiveX 控件拖动到窗体上) 
...
3. 在“生成”菜单中，单击“生成解决方案”。
   ...
   现在，按 F5 时，应用程序按预期运行，但现在它在免注册 COM 下运行。 为了证明这一点，请尝试注销 VB6Hello.dll 组件，RegFreeComDemo1.exe IDE Visual Studio运行。 这一次单击按钮时，它仍可正常工作。 如果暂时重命名应用程序清单，它将再次失败。
...
## <a name="reference-native-assemblies"></a>引用本机程序集
 Visual Studio 6.0 Visual Basic C++ 程序集的本机引用;此类引用称为本机引用。 可以通过验证引用的"文件类型"属性是否设置为 Native或 **ActiveX。** 

 若要添加本机引用，请使用 **"添加引用** "命令，然后浏览到清单。 某些组件将清单放在 DLL 中。 在这种情况下，只需选择 DLL 本身，如果Visual Studio组件包含嵌入清单，则添加它作为本机引用。 Visual Studio也将自动包括清单中列出的任何依赖文件或程序集（如果它们与引用的组件在同一文件夹中）。
...
## <a name="limitations-of-registration-free-com-component-deployment"></a>免注册 COM 组件部署的限制
 与传统的部署技术相比，免注册 COM 具有明显优势。 但是，还应指出一些限制和注意事项。最大的限制是，它仅适用于 Windows XP 或更高版本。 实现免注册 COM 需要更改组件在核心操作系统中的加载方式。 遗憾的是，无注册 COM 没有下层支持层。

 并非每个组件都是免注册 COM 的合适候选项。 如果以下任一项为 true，则组件不适合：

- 组件是进程外服务器。 不支持 EXE 服务器;仅支持 DLL。

- 组件是操作系统的一部分，或者是一个系统组件，如 XML、Internet Explorer 或 Microsoft 数据访问组件 (MDAC) 。 应遵循组件作者的重新分发策略;请咨询供应商。

- 组件是应用程序的一部分，例如Microsoft Office。 例如，不应尝试隔离对象Microsoft Excel模型。 这是 Office的一部分，只能在安装了完整产品Office计算机上使用。

- 组件旨在用作外接程序或管理单元，例如，Office或 Web 浏览器中的控件。 此类组件通常需要托管环境定义的某种注册方案，而该环境超出了清单本身的范围。

- 组件管理系统的物理或虚拟设备，例如，打印后台处理程序的设备驱动程序。

- 组件是数据访问可再发行组件。 数据应用程序通常需要安装单独的数据访问可再发行组件，然后才能运行。 不应尝试隔离 Microsoft ADO 数据控制、Microsoft OLE DB 或 Microsoft Data Access Components (MDAC) 。 相反，如果应用程序使用 MDAC 或 SQL Server Express，应将其设置为先决条件;请参阅[如何：使用应用程序安装ClickOnce必备组件](../deployment/how-to-install-prerequisites-with-a-clickonce-application.md)。

  在某些情况下，组件的开发人员可能会重新设计该组件，实现免注册 COM。 如果无法这样做，仍可使用引导程序通过标准注册方案生成和发布依赖于这些应用程序的应用程序。 有关详细信息，请参阅 [Creating Bootstrapper Packages](../deployment/creating-bootstrapper-packages.md)。

  每个应用程序只能隔离 COM 组件一次。 例如，无法将同一 COM 组件与属于同一应用程序的两个不同的类库项目隔离。 这样做将导致生成警告，并且应用程序将无法运行时加载。 为了避免此问题，Microsoft 建议将 COM 组件封装在单个类库中。

  在某些情况下，需要在开发人员计算机上注册 COM，即使应用程序的部署不需要注册。 属性要求在开发人员计算机上注册 COM 组件，以便 `Isolated` 自动生成生成期间清单。 生成期间没有调用自注册的注册捕获功能。 此外，类型库中未显式定义的任何类将不会反映在清单中。 将 COM 组件与预先存在的清单（如本机引用）一起使用时，该组件可能无需在开发时注册。 但是，如果组件是一个ActiveX，并且你想要在"工具箱"和"窗体"设计器Windows注册。

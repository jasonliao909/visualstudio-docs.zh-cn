...
在 **表设计器** 中，打开实体的快捷菜单 (单个行) 或属性 (单个单元格) ，然后选择"编辑 **"。**
...
当服务器资源管理器连接到存储帐户或从存储帐户获取数据时，可能需要长达一分钟的时间才能完成操作。 如果服务器资源管理器无法连接，操作可能会退出。检索数据时，可以继续在数据的其他部分Visual Studio。 如果花费了太长时间想要取消操作，请选择服务器资源管理器工具栏上的“停止刷新”按钮。

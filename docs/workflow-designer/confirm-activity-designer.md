...
description: 了解 Confirm 活动设计器，以及如何使用此设计器来创建和配置 Confirm 活动。
...
### <a name="using-the-confirm-activity-designer"></a>使用 Confirm 活动设计器
 可以在“工具箱”的“事务”类别中找到 Confirm 活动设计器，通过单击工作流设计器左侧的“工具箱”选项卡来访问。 或者，从“视图”菜单中选择“工具箱”或按 Ctrl+Alt+X    。

 可以将 Confirm 活动设计器从“工具箱”拖放到工作流设计器图面上通常放置活动的任何位置，如 <xref:System.Activities.Statements.Sequence> 内。 这将创建具有 Confirm 的默认 <xref:System.Activities.Statements.Confirm> 的 <xref:System.Activities.Activity.DisplayName%2A> 活动。 可以在 Confirm 活动设计器的标头中或在属性网格的“DisplayName”框中编辑 <xref:System.Activities.Activity.DisplayName%2A> 值。

...
|<xref:System.Activities.Statements.Transition.Condition%2A>|错误|如果存在，则指定一个表达式，该表达式在将控件传递给目标状态之前，必须计算结果为 True。 可以在属性网格和扩展转换设计器中编辑此条件。 共享转换中的多个条件按显示在转换设计器中的顺序进行评估。 **注意：** 请注意，如果转换的 计算结果为 False (或者共享触发器转换的所有条件评估为 <xref:System.Activities.Statements.Transition.Condition%2A> **False**) ，则转换不会发生，并且将重新计划从状态转换的所有触发器。 在本教程中，由于配置条件的方式，这种情况不会发生（我们针对猜测是正确或者错误提供了具体的操作）。|

...
ms.locfileid: "137667429"
---
# <a name="edit-python-code"></a>编辑 Python 代码

由于将大量开发时间都用在了代码编辑器中，因此可借助 [Visual Studio 中的 Python 的支持](installing-python-support-in-visual-studio.md)中的功能来提高工作效率。 功能包括 IntelliSense 语法突出显示、自动完成、签名帮助、方法重写、搜索和导航。
...
Visual Studio 2017 版本15.6 和更高版本使用不同的方法来提供不依赖于数据库的 IntelliSense 完成。
...
代码着色使用代码分析中的信息为变量、语句和代码的其他部分着色。 例如，引用模块或类的变量可能采用不同于函数或其他值的颜色显示，而参数名称将采用不同于局部变量或全局变量的颜色显示。  (默认情况下，函数不以粗体显示) ：
...
例如，`class` 是一个插入类定义的代码片段的快捷方式。 键入 时，会看到代码片段显示在自动完成列表中 `class` ：

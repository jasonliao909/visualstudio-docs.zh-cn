...
conda 环境是使用 工具或 `conda` 2017 Visual Studio 15.7 及更高版本中的集成 conda 管理创建的环境。  (需要 Anaconda 或 Miniconda（通过 Visual Studio 安装程序提供）的信息，请参阅安装[Visual Studio 2017](installing-python-support-in-visual-studio.md#visual-studio-2017)

::: moniker range="vs-2017"

1. 选择“Python 环境”窗口中的“+ 创建 Conda 环境”，这将打开“新建 Conda 环境”选项卡    ：

   ![新 conda 环境的"创建"选项卡-1](media/environments/environments-conda-1.png)
...
1. 在"添加环境"对话框中，选择 **"Conda 环境"** 选项卡：
   ...
   !["添加环境"对话框中的"Conda 环境"选项卡-2019](media/environments/environments-conda-1-2019.png)
...
1. 配置以下字段：
    ...
    | 在“Python 环境”窗口中查看 | 指定在创建环境后是否显示 **"Python** 环境"窗口。 |
...
1. 在"添加环境"对话框中，选择 **"Conda 环境"** 选项卡：
   ...
   !["添加环境"对话框中的"Conda 环境"选项卡-2022](media/environments/environments-conda-1-2022.png)
...
> [!Note]
> 为获得 Conda 环境的最佳使用结果，请使用 Conda 4.4.8 或更高版本（Conda 版本与 Anaconda 版本不同）。 可以通过 Visual Studio 安装程序安装适用于 Miniconda (Visual Studio 2019 和 Visual Studio 2022) 和 Anaconda (Visual Studio 2017) 的版本。
...
由于 conda 环境不随项目一起存储，因此它们的行为类似于全局环境。 例如，将新包安装到 Conda 环境，从而使该包适用于使用此环境的所有项目。
...
1. 如果需要删除手动标识的环境，请在"配置 **"选项卡上** 选择"删除 **"** 命令。自动检测的环境不提供此选项。 有关更多信息，请参阅[配置选项卡](python-environments-window-tab-reference.md#configure-tab)。
...
1. 在"添加环境"对话框中，选择" **现有环境"** 选项卡：
   ...
   !["添加环境"对话框中的现有环境选项卡-2019](media/environments/environments-custom-1-2019.png)

1. 选择“环境”  下拉列表，然后选择“自定义”  ：

   !["添加环境"对话框中的"自定义环境"选项-2019](media/environments/environments-custom-2-2019.png)

1. 在对话框中提供的字段中，输入或浏览 (前缀路径下) 解释器的路径，该路径将填充大多数其他字段。 

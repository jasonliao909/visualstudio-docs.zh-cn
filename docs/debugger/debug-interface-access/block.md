...
description: 查找有关 SymTagBlock (块符号) 的信息，该类型标识 Visual Studio 调试接口访问 SDK 中函数中的嵌套范围。
...
---
# <a name="block"></a>阻止
每个代码块由符号 `SymTagBlock` 标识。 块符号用于标识函数中的嵌套范围。
...
|[IDiaSymbol::get_name](../../debugger/debug-interface-access/idiasymbol-get-name.md)|`BSTR`|返回块名称，该 (通常是空字符串) 。|
...
|[IDiaSymbol::get_symTag](../../debugger/debug-interface-access/idiasymbol-get-symtag.md)|`DWORD`|返回 `SymTagBlock` ([SymTagEnum 枚举值之一) 。](../../debugger/debug-interface-access/symtagenum.md)|

...
author: ghogen
ms.author: ghogen
ms.openlocfilehash: 9eb81ca022e3998ce847566ca852e5c5c3ae5d26
ms.sourcegitcommit: a439b1878939b2364cee0d09b851c2a67c42e563
ms.translationtype: MT
ms.contentlocale: zh-CN
ms.lasthandoff: 02/12/2022
ms.locfileid: "138565988"
---
...
- VS Code已安装 Bridge to Kubernetes [扩展](https://aka.ms/bridge-to-k8s-vsc-extension)。
...
在VS Code中，Bridge to Kubernetes支持所有语言，只要可以在本地运行它们。 在 Visual Studio 中，Bridge to Kubernetes支持 .NET Core。 Bridge to Kubernetes不支持 .NET Framework Visual Studio，因为它需要Windows节点支持。
...
> [!NOTE]
> 在连接到群集期间，加上 15 分钟，Bridge to Kubernetes在本地计算机上运行名为 *EndpointManager* 的进程。
...
- 验证 Kubernetes 群集是否未启用Azure Dev Spaces。
...
- 使用子域的 GENERATED_NAME 复制命名空间中发现的所有入口（ *包括负载均衡* 器入口）。
...
![已启用群集的Bridge to Kubernetes图。](media/bridge-to-kubernetes/kubernetes-cluster-dev-computer.svg)

当群集收到包含 GENERATED_NAME 域的请求时，它会向请求添加 *kubernetes-route-as=* GENERATED_NAME标头。 Envoy pod 负责将该请求路由到群集中的相应服务。 对于对独立处理的服务的请求，群集会将请求重定向到远程代理的开发计算机。

当群集收到请求而不GENERATED_NAME域时，它不会向请求添加标头。 Envoy pod 负责将该请求路由到群集中的相应服务。 对于要替换的服务的请求，Pod 会将请求路由到原始服务而不是远程代理。
...
从群集断开连接时，默认情况下，Bridge to Kubernetes删除所有 envoy Pod 和重复服务。

> [!NOTE]
> 路由管理器部署和服务在命名空间中保持运行。 要删除部署和服务，请对命名空间运行以下命令。
>
> ```azurecli
...
使用 Bridge to Kubernetes连接到群集时，计算机会记录诊断信息。 它将它们存储在开发计算机的 *TEMP* 目录中的 *Bridge to Kubernetes 文件夹中。*

### <a name="kubernetes-rbac-authorization"></a>Kubernetes RBAC 授权

Kubernetes 提供了基于角色的访问控制 (RBAC) 来管理用户和组的权限。 有关信息，请参阅 [Kubernetes 文档](https://kubernetes.io/docs/reference/access-authn-authz/rbac/)。 可以通过创建 YAML `kubectl` 文件，然后使用 将启用 RBAC 的群集应用到群集来设置其权限。

若要设置对群集的权限，请创建或修改 YAML 文件，例如 *permissions.yml*。 使用 的命名空间 `<namespace>` 以及需要访问权限的用户和组。
...
- 目前，Bridge to Kubernetes pod 必须是 Linux 容器。 Windows容器。
